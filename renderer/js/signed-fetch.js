...
export async function signedFetch(path, init = {}) {
    ...
    // Sign the URL the backend reconstructs (https://nosmero.com/... in the
    // Nosdag shell) so the server's `u`-tag check matches; send the request to
    // the local origin so the shell's app:// proxy forwards /api/* without a
    // cross-origin call. On the web both resolve to the same URL — see
    // api-origin.js.
    const auth = await buildAuthHeader(signedUrl(path), method, init.body);
