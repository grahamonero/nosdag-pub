...
async function sendRequest(method, params = []) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Encrypt request using NIP-44 (nostr-tools from the local bundle — no CDN dependency)
...
async function handleResponse(event) {
    try {
        ...
        // Decrypt response using NIP-44 (nostr-tools from the local bundle — no CDN dependency)
        const nip44Module = window.NostrTools.nip44;
        ...
        // Never log the plaintext: NIP-46 responses carry signed events and decrypted content
        const response = JSON.parse(plaintext);
        console.log('📥 NIP-46 response:', response.id, response.error ? '(error: ' + response.error + ')' : '(ok)');
...
export async function connect(bunkerURI) {
    try {
        ...
        // nostr-tools utilities from the local bundle — no CDN dependency
