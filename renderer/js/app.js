...
// Nosdag: mixed-posture repair. Once we know who's logged in and their chain head, ask main to
// pull any chain segments THIS posture's store is missing from the other posture's at-rest store
// (quit-relaunch posture changes never migrate — only in-app switches do). Cheap no-op once the
// head is verified for this posture; retried briefly at boot in case the node is still starting.
function nosdagCatchUp(pubkey, attempt = 0) {
    try {
        if (!window.nosdag?.migrate?.catchUp || !pubkey) return;
        const headCid = localStorage.getItem('nosdag:head:' + pubkey);
        const archiveCid = localStorage.getItem('nosdag:archive:' + pubkey);
        if (!headCid && !archiveCid) return;
        window.nosdag.migrate.catchUp({ pubkey, headCid, archiveCid }).then((r) => {
            if (r?.error === 'node not ready' && attempt < 5) {
                setTimeout(() => nosdagCatchUp(pubkey, attempt + 1), 5000);
            } else if (r?.notes || r?.blocks) {
                console.log(`[nosdag] catch-up healed ${r.notes} notes + ${r.media} media (${r.blocks} blocks) into this posture's store`);
            } else if (r?.error) {
                console.warn('[nosdag] catch-up:', r.error);
            }
        }).catch(() => {});
    } catch { /* shell only */ }
}
window.addEventListener('nosmero:login', (e) => nosdagCatchUp(e.detail?.pubkey));
document.addEventListener('DOMContentLoaded', () => setTimeout(() => nosdagCatchUp(State.publicKey), 3000));
...
async function initializeApp() {
    ...
    try {
        ...
        Utils.showNotification('Starting Nosdag...', 'info');
        ...
        Utils.showNotification('Nosdag loaded successfully!', 'success');
        console.log('Nosdag ready!');
        ...
        Utils.showNotification('Failed to start Nosdag: ' + error.message, 'error');
        showErrorFallback();
    }
}
...
async function checkExistingSession() {
    ...
    // Check for a signer marker if no encrypted session found. Only marker values are
    // accepted from this localStorage key — raw key material found there is ignored
    // (and never written by any current login path); the user re-logs-in and the key
    // is stored PIN-encrypted. The value is deliberately not deleted: it could be
    // someone's only copy of their key.
    if (!storedPrivateKey) {
        const marker = localStorage.getItem('nostr-private-key');
        if (marker === 'extension' || marker === 'amber' || marker === 'nsec-app') {
            storedPrivateKey = marker;
        } else if (marker) {
            console.warn('⚠️ Ignoring legacy plaintext key in localStorage — log in again to store it encrypted');
        }
    }
    ...
    if (storedPrivateKey) {
        ...
        if (storedPrivateKey === 'extension') {
            ...
            // The bunker URI is stored OS-keychain-encrypted (safeStorage). A legacy plaintext
            // copy is migrated into the keychain on first sight; without a keychain it is kept
            // as-is so an existing Amber session keeps working.
            let bunkerURI = null;
            const encUri = localStorage.getItem('amber-bunker-uri-enc');
            if (encUri) {
                const dec = await window.nosdag?.secrets?.decrypt(encUri).catch(() => null);
                bunkerURI = dec?.plaintext || null;
                if (!bunkerURI) console.warn('⚠️ Could not decrypt stored bunker URI (keychain changed?) — reconnect to Amber');
            } else {
                const legacyUri = localStorage.getItem('amber-bunker-uri');
                if (legacyUri) {
                    bunkerURI = legacyUri;
                    try {
                        const enc = await window.nosdag?.secrets?.encrypt(legacyUri);
                        if (enc?.data) {
                            localStorage.setItem('amber-bunker-uri-enc', enc.data);
                            localStorage.removeItem('amber-bunker-uri');
                            console.log('🔐 Migrated bunker URI into the OS keychain');
                        }
                    } catch (e) {
                        console.warn('Bunker URI keychain migration failed:', e?.message);
                    }
                }
            }
            if (bunkerURI) {
...
async function startApplication() {
    ...
    console.log('  - has signing key:', !!State.getPrivateKeyForSigning());
...
async function handleNavigation(event) {
    ...
    // Every page renders inside .main, the single shared scroll container. Without a reset,
    // the previous view's scroll offset carries over and the next view opens with its title
    // already scrolled under the fixed top bar. Thread back-restore bypasses this path.
    document.querySelector('.app-container .main')?.scrollTo(0, 0);

    // The "show my own notes" quick-toggle chip belongs to the Following feed only — hide it
    // on any page navigation. Plain 'home' lands logged-in users on the Following feed, so it
    // shows there; a feed-tab switch (skipFeedLoad) re-decides it per feed in ui/navigation.js.
    const followingChip = document.getElementById('followingFeedChip');
    if (followingChip) {
        const chipSkipFeedLoad = event.currentTarget.dataset.skipFeedLoad === 'true';
        followingChip.style.display = (tab === 'home' && !chipSkipFeedLoad && State.publicKey) ? 'block' : 'none';
    }
    ...
    const hostedFollowsPage = document.getElementById('hostedFollowsPage');
    if (hostedFollowsPage) {
        hostedFollowsPage.style.display = 'none';
    }

    const anonModePage = document.getElementById('anonModePage');
    if (anonModePage) {
        anonModePage.style.display = 'none';
    }

    const mediaLibraryPage = document.getElementById('mediaLibraryPage');
    if (mediaLibraryPage) {
        mediaLibraryPage.style.display = 'none';
    }
    ...
    switch (tab) {
        ...
        case 'notifications':
            ...
            break;
        case 'profile':
            // The modern profile surface (ui/profile.js) — the legacy loadUserProfile page
            // had its own count/notes fetchers with no local fallbacks, so the rail's own
            // profile showed zeros whenever relays were silent (cold Tor start) while
            // other-user profiles (already on this surface) worked.
            await window.viewUserProfilePage(State.publicKey, true);
            break;
        case 'settings':
        ...
        case 'node':
            ...
            break;
        case 'hosted':
            await loadHostedFollowsPage();
            break;
        case 'anon':
            await loadAnonModePage();
            break;
        case 'media':
            await loadMediaLibraryPage();
            break;
        default:
...
// My Node page — node status, media library, and hosted follows in one tabbed surface.
// The node tab needs no login (it's the local node); media/hosted gate themselves.
async function loadMyNodePage(tab = 'node') {
    State.setCurrentPage(tab);

    const feed = document.getElementById('feed');
    if (feed) feed.style.display = 'none';

    const page = document.getElementById('nodePage');
    if (!page) return;
    page.style.display = 'block';

    try {
        const MN = await import('./nosdag/my-node.js');
        await MN.renderMyNode(page, tab);
    } catch (e) {
        console.error('[MyNode] page failed to load:', e);
        page.innerHTML = `<div style="padding:24px;color:#f87171;">My Node failed: ${e.message || e}</div>`;
    }
}
window.loadMyNodePage = loadMyNodePage;

async function loadNodePage() { return loadMyNodePage('node'); }
window.loadNodePage = loadNodePage;

async function loadHostedFollowsPage() { return loadMyNodePage('hosted'); }
window.loadHostedFollowsPage = loadHostedFollowsPage;

// Load Anonymous Mode page — Tor/clearnet switch + onion relay picker (no login required)
async function loadAnonModePage() {
    State.setCurrentPage('anon');

    const feed = document.getElementById('feed');
    if (feed) feed.style.display = 'none';

    const page = document.getElementById('anonModePage');
    if (!page) return;
    page.style.display = 'block';
    page.innerHTML = '<div style="padding:48px;text-align:center;color:#888;">Loading…</div>';

    try {
        const T = await import('./nosdag/tor-mode.js');
        await T.renderAnonModePage(page);
    } catch (e) {
        console.error('[AnonMode] page failed to load:', e);
        page.innerHTML = `<div style="padding:24px;color:#f87171;">Anonymous Mode failed: ${e.message || e}</div>`;
    }
}
window.loadAnonModePage = loadAnonModePage;

// H2 Tor kill-switch banner: if the Tor daemon dies in anonymous mode, main blackholes all egress
// (fail-closed) and fires 'tor:down'. Show a persistent, blocking banner on whatever page is open so
// the user knows the network is cut and to restart. The security guarantee is main's blackhole; this
// is the user-facing half. Registered once on app load.
function installTorDownBanner() {
    if (!window.nosdag?.mode?.onDown) return; // not in the shell (web/dev)
    const showBanner = () => {
        if (document.getElementById('nd-tor-down-banner')) return;
        const bar = document.createElement('div');
        bar.id = 'nd-tor-down-banner';
        bar.style.cssText = 'position:fixed;left:0;right:0;top:0;z-index:99999;padding:12px 18px;'
            + 'background:#3a0d0d;border-bottom:1px solid #ff5d5d;color:#ffd9d9;text-align:center;'
            + "font:600 13px/1.4 'SF Mono','Cascadia Code',ui-monospace,Menlo,monospace";
        bar.textContent = '⚠ Tor stopped — all network access is blocked to protect your IP. Restart Nosdag to reconnect.';
        document.body.appendChild(bar);
    };
    window.nosdag.mode.onDown(showBanner);
    // The banner is DOM-only, so a page reload wipes it — but main latches torDown until restart.
    // Re-show it on load if the kill-switch is still engaged (egress stays blackholed regardless).
    window.nosdag.kubo?.status?.().then((s) => { if (s?.torDown) showBanner(); }).catch(() => {});
}
installTorDownBanner();

// If the node backend fails to start — most often Tor not being installed when Anonymous mode was
// chosen, or a tor bootstrap timing out on a first run — surface a clear, actionable banner. The
// error otherwise only hits the console and the app just looks broken. The watch window must
// outlast tor's bootstrap ceiling (180s), which only fails minutes after the page loads.
function installNodeErrorBanner() {
    if (!window.nosdag?.kubo?.status) return;
    let tries = 0;
    const check = async () => {
        if (document.getElementById('nd-node-error-banner') || tries++ > 100) return;
        let s; try { s = await window.nosdag.kubo.status(); } catch { setTimeout(check, 2500); return; }
        if (s?.ready) return;                              // node came up — nothing to surface
        const err = s?.nodeStartError;
        if (!err) { setTimeout(check, 2500); return; }     // still booting — keep watching
        const bar = document.createElement('div');
        bar.id = 'nd-node-error-banner';
        bar.style.cssText = 'position:fixed;left:0;right:0;top:0;z-index:99999;padding:12px 18px;'
            + 'background:#2e2410;border-bottom:1px solid #f5a623;color:#ffe9c2;text-align:center;'
            + "font:600 13px/1.5 'SF Mono','Cascadia Code',ui-monospace,Menlo,monospace";
        ...
        const smallBtn = (label) => {
            ...
            b.textContent = label;
            b.style.cssText = 'margin-left:8px;padding:4px 12px;border:1px solid #f5a623;background:transparent;color:#f5a623;border-radius:6px;cursor:pointer;font:inherit';
            return b;
        };
        if (err.kind === 'tor-timeout' && window.nosdag?.mode?.set) {
            // mode.set('tor') with the node down performs a real stop→start; tor resumes from the
            // consensus progress it saved, so the retry usually crosses 100% quickly.
            const rb = smallBtn('Retry Tor');
            rb.onclick = async () => {
                rb.disabled = true; rb.textContent = 'Retrying…';
                const tick = setInterval(async () => {
                    try {
                        const st = await window.nosdag.kubo.status();
                        if (st?.torBootstrap != null && !st.ready) rb.textContent = `Retrying… ${st.torBootstrap}%`;
                    } catch { /* keep last label */ }
                }, 1500);
                try {
                    const r = await window.nosdag.mode.set('tor', { retry: true });
                    clearInterval(tick);
                    if (r?.ok) { window.nosdag.app?.reload?.(); return; }
                    msg.textContent = '⚠ ' + (r?.error || err.message) + ' ';
                    rb.disabled = false; rb.textContent = 'Retry Tor';
                } catch (e) {
                    clearInterval(tick);
                    rb.disabled = false; rb.textContent = 'Retry Tor';
                }
        ...
        if ((err.kind === 'no-tor' || err.kind === 'tor-timeout') && window.nosdag?.mode?.set) {
            const btn = smallBtn('Use clearnet');
            btn.onclick = async () => {
                btn.disabled = true; btn.textContent = 'Switching…';
                try { await window.nosdag.mode.set('clearnet'); window.nosdag.app?.reload?.(); }
                catch { btn.disabled = false; btn.textContent = 'Use clearnet'; }
            };
            bar.appendChild(btn);
        }
        document.body.appendChild(bar);
...
installNodeErrorBanner();

// M7: copy a secret (seed phrase / nsec) to the clipboard, then auto-clear it after a delay so it
// doesn't linger in the OS clipboard (or clipboard history) for malware to scrape. Only clears if
// the clipboard still holds the secret, so it won't clobber something copied since. (JS strings are
// immutable, so the in-memory copy itself can't be wiped — this bounds the clipboard exposure.)
window.copySecret = async function (text, clearAfterMs = 60000) {
    try { await navigator.clipboard.writeText(text); } catch { return; }
    setTimeout(async () => {
        try { if (await navigator.clipboard.readText() === text) await navigator.clipboard.writeText(''); }
        catch { /* clipboard read may be denied — leave it as-is rather than risk clobbering */ }
    }, clearAfterMs);
};

// External links in notes/profiles render as <a target="_blank" href="http…">. Intercept the click
// in the capture phase so it (a) doesn't bubble into the note's thread-open handler (which looked
// like "clicking a link reloads the thread"), and (b) routes through main: open in the OS browser on
// clearnet, or — in Tor mode — copy the URL + explain, since opening your normal browser would
// bypass Tor and leak your IP.
document.addEventListener('click', async (e) => {
    const a = e.target?.closest?.('a[href]');
    if (!a) return;
    const href = a.getAttribute('href') || '';
    if (!/^https?:\/\//i.test(href)) return; // only external http(s) links — internal routing is untouched
    e.preventDefault();
    e.stopPropagation();
    if (!window.nosdag?.app?.openExternal) { window.open(href, '_blank', 'noopener'); return; } // web/dev fallback
    const r = await window.nosdag.app.openExternal(href);
    if (r?.blocked === 'tor') {
        try { await navigator.clipboard.writeText(href); } catch { /* clipboard denied */ }
        const notify = window.NostrUtils?.showNotification || window.showNotification;
        notify?.('Link copied — opening it would use your normal browser and bypass Tor. Paste it into Tor Browser to visit safely.', 'info');
    }
}, true);

// Media library lives in the My Node page now; the tab shows a sign-in gate when logged out
async function loadMediaLibraryPage() { return loadMyNodePage('media'); }
window.loadMediaLibraryPage = loadMediaLibraryPage;
...
async function loadUserProfile() {
    ...
    if (feed) {
        feed.innerHTML = `
            <div style="max-width: 800px; margin: 0 auto; padding: 20px;">
                <div id="profileHeader" style="background: linear-gradient(135deg, color-mix(in srgb, var(--nd-accent) 10%, transparent), color-mix(in srgb, var(--nd-accent) 10%, transparent)); border: 1px solid #333; border-radius: 16px; padding: 24px; margin-bottom: 24px;">
                ...
                <div style="display: flex; gap: 12px; margin-bottom: 24px; flex-wrap: wrap;">
                    <button id="profileTabPosts" class="profile-tab active" onclick="switchProfileTab('posts')"
                            style="padding: 10px 20px; border-radius: 20px; border: 1px solid #333; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); color: #000; cursor: pointer; font-weight: bold;">
                        Posts
                    </button>
...
function displayProfileHeader(profile) {
    ...
    profileHeader.innerHTML = `
        <div style="display: flex; align-items: flex-start; gap: 20px; flex-wrap: wrap;">
            ${profile.picture ? 
                `<img src="${profile.picture}" style="width: 80px; height: 80px; border-radius: 50%; border: 3px solid var(--nd-accent);" 
                     onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                 <div style="width: 80px; height: 80px; border-radius: 50%; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); display: none; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 24px; border: 3px solid var(--nd-accent);">${displayName.charAt(0).toUpperCase()}</div>` : 
                `<div style="width: 80px; height: 80px; border-radius: 50%; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 24px; border: 3px solid var(--nd-accent);">${displayName.charAt(0).toUpperCase()}</div>`
            }
            ...
            <div style="flex-grow: 1; min-width: 0;">
                ...
                ${profile.website ? `<p style="margin: 8px 0 0 0;"><a href="${profile.website}" target="_blank" style="color: var(--nd-accent); text-decoration: none;">🔗 ${profile.website}</a></p>` : ''}
                ...
                <div style="margin-top: 16px; display: flex; gap: 20px; align-items: center; justify-content: space-between; flex-wrap: wrap;">
                    ...
                    <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                        <button onclick="showEditProfileModal()" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #000; padding: 10px 20px; border-radius: 8px; cursor: pointer; font-weight: bold; font-size: 14px;">
                            ✏️ Edit Profile
                        </button>
                        <button onclick="copyUserNpub('${State.publicKey}')" style="background: color-mix(in srgb, var(--nd-accent) 20%, transparent); border: 1px solid var(--nd-accent); border-radius: 8px; color: var(--nd-accent); padding: 10px 20px; cursor: pointer; font-weight: bold; font-size: 14px;">
                            📋 Copy npub
                        </button>
                    </div>
                </div>
            ...
            addressContainer.innerHTML = `
                <div style="color: #666; font-size: 12px; padding: 8px 0;">
                    ...
                    <button onclick="handleNavItemClick('settings')" 
                            style="background: none; border: none; color: var(--nd-accent); cursor: pointer; text-decoration: underline; font-size: 12px;">
                        Add in Settings
                    </button>
                </div>
            `;
        ...
        try {
            ...
            const renderedPosts = await Promise.all(posts.map(async post => {
                try {
                    ...
                    const content = post.content.length > 500 ? 
                        post.content.substring(0, 500) + '... <span style="color: var(--nd-accent); cursor: pointer;">Read more</span>' : 
                        post.content;
                    ...
                    return `
                        <div style="background: rgba(255, 255, 255, 0.02); border: 1px solid #333; border-radius: 12px; padding: 20px; margin-bottom: 16px;">
                            <div style="display: flex; align-items: flex-start; gap: 12px; margin-bottom: 12px;">
                                ${userProfile.picture ? 
                                    `<img src="${userProfile.picture}" style="width: 40px; height: 40px; border-radius: 50%;" 
                                         onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                                     <div style="width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); display: none; align-items: center; justify-content: center; color: white; font-weight: bold;">${(userProfile.name || 'A').charAt(0).toUpperCase()}</div>` : 
                                    `<div style="width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); display: flex; align-items: center; justify-content: center; color: white; font-weight: bold;">${(userProfile.name || 'A').charAt(0).toUpperCase()}</div>`
                                }
                            ...
                            <div style="display: flex; gap: 20px; padding-top: 12px; border-top: 1px solid #333; font-size: 14px;">
                                <span style="color: #888; cursor: pointer;" onmouseover="this.style.color='var(--nd-accent)'" onmouseout="this.style.color='#888'" onclick="NostrPosts.replyToPost('${post.id}')">
                                    💬 Reply
                                </span>
                                <span style="color: #888; cursor: pointer;" onmouseover="this.style.color='var(--nd-accent)'" onmouseout="this.style.color='#888'" onclick="NostrPosts.repostNote('${post.id}')">
                                    🔄 Repost
                                </span>
                                <span style="color: #888; cursor: pointer;" onmouseover="this.style.color='var(--nd-accent)'" onmouseout="this.style.color='#888'" onclick="NostrPosts.likePost('${post.id}')">
                                    ❤️ Like
                                </span>
                                <span style="color: #888; cursor: pointer;" onmouseover="this.style.color='var(--nd-accent)'" onmouseout="this.style.color='#888'" onclick="sharePost('${post.id}')">
                                    📤 Share
                                </span>
                            </div>
                        </div>
            ...
            const loadMoreButton = hasMorePosts ? `
                <div id="ownProfileLoadMoreContainer" style="text-align: center; padding: 20px; border-top: 1px solid #333;">
                    <button onclick="loadMoreOwnPosts()" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #fff; padding: 12px 24px; border-radius: 8px; font-size: 16px; font-weight: bold; cursor: pointer;">
            ...
            try {
                ...
                if (pinnedIds.length > 0) {
                    ...
                    if (pinnedEvents.length > 0) {
                        ...
                        pinnedHtml = `
                            <div style="color: var(--nd-accent); font-size: 13px; font-weight: 600; padding: 8px 16px; margin-bottom: 8px; display: flex; align-items: center; gap: 6px;">
                                📌 Pinned
                            </div>
...
function switchProfileTab(tab) {
    ...
    if (activeBtn) {
        activeBtn.style.background = 'linear-gradient(135deg, var(--nd-accent), var(--nd-accent))';
        ...
        case 'about':
            ...
            profileContent.innerHTML = `
                <div style="background: rgba(255, 255, 255, 0.02); border: 1px solid #333; border-radius: 12px; padding: 24px;">
                    <h3 style="color: var(--nd-accent); margin-bottom: 16px;">About</h3>
                    ...
                    <h3 style="color: var(--nd-accent); margin-bottom: 16px;">Details</h3>
                    <div style="color: #ccc;">
                        ...
                        ${userProfile.website ? `<p style="margin-bottom: 8px;"><strong>Website:</strong> <a href="${userProfile.website}" target="_blank" style="color: var(--nd-accent);">${userProfile.website}</a></p>` : ''}
                    </div>
                </div>
            `;
            break;
...
async function loadSettings_OLD_DISABLED() {
    ...
    feed.innerHTML = `
        <div style="padding: 20px; max-width: 800px;">
            <h2 style="color: var(--nd-accent); margin-bottom: 30px;">⚙️ Settings</h2>
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <div style="display: flex; gap: 12px;">
                        <button id="darkThemeBtn" onclick="setTheme('dark')"
                                style="padding: 12px 24px; border: none; border-radius: 8px; cursor: pointer; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); color: #000; font-weight: bold;">
                            🌙 Dark
                        </button>
            ...
            <div class="settings-section-card" style="margin-bottom: 40px;">
                ...
                <div style="margin-bottom: 20px; padding: 15px; background: color-mix(in srgb, var(--nd-accent) 10%, transparent); border-radius: 8px; border-left: 3px solid var(--nd-accent);">
                ...
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <button onclick="publishRelayList()"
                            style="padding: 10px 20px; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #000; border-radius: 6px; cursor: pointer; font-weight: bold;">
                        📢 Publish Relay List
                    </button>
            ...
            <div class="settings-section-card" style="margin-bottom: 40px;">
                ...
                <div style="margin-top: 15px;">
                    <button onclick="exportPrivateKey()"
                            style="padding: 10px 20px; background: var(--nd-accent); border: none; color: #000; border-radius: 6px; cursor: pointer; font-weight: bold;">
                        🔑 Export Private Key
                    </button>
                </div>
            </div>
            ...
            <div class="settings-section-card" style="margin-bottom: 40px;">
                ...
                <!-- Monero Settings -->
                <div style="margin-bottom: 25px; padding: 15px; background: color-mix(in srgb, var(--nd-accent) 10%, transparent); border-radius: 8px; border-left: 3px solid var(--nd-accent);">
                    <h4 style="color: var(--nd-accent); margin: 0 0 15px 0;">💰 Monero (XMR) Zaps</h4>
                    <div style="margin-bottom: 15px;">
            ...
            <div class="settings-section-card">
                ...
                <div style="color: var(--text-secondary); line-height: 1.6;">
                    <p><strong style="color: var(--nd-accent);">Nosdag</strong> - Modular Architecture</p>
                    ...
                    <div style="margin-top: 20px; padding: 15px; background: var(--bg-hover); border-radius: 8px;">
                        <h4 style="color: var(--nd-accent); margin: 0 0 10px 0;">Key Features:</h4>
...
function showAuthUI() {
    ...
    feed.innerHTML = `
        <div style="padding: 40px; text-align: center; max-width: 500px; margin: 0 auto;">
            <h2 style="color: var(--nd-accent); margin-bottom: 30px;">Welcome to Nosdag</h2>
            ...
            <div style="display: flex; flex-direction: column; gap: 16px; margin-bottom: 30px;">
                <button onclick="createNewAccount()" 
                        style="padding: 16px 24px; border: none; border-radius: 12px; cursor: pointer; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); color: #000; font-weight: bold; font-size: 16px;">
                    🎭 Create New Account
                </button>
    ...
    feed.innerHTML = `
        <div style="padding: 40px; text-align: center;">
            ...
            <p style="color: #ccc; margin-bottom: 30px;">
                Nosdag failed to start properly. Please try refreshing the page.
            </p>
    ...
    if (confirm('Are you sure you want to export your private key? Keep it secure!')) {
        try {
            ...
            modal.style.cssText = `
                ...
                z-index: 10000;
            `;
            const accountLabel = (window.State?.userMetadata?.username || window.State?.userMetadata?.name || 'nosdag').replace(/[^a-zA-Z0-9_-]/g, '');
            modal.innerHTML = `
                <div style="background: #1a1a1a; padding: 30px; border-radius: 12px; max-width: 500px; text-align: center;">
                    <h3 style="color: var(--nd-accent); margin-bottom: 16px;">🔑 Your Private Key</h3>
                    ...
                    <div style="margin-bottom: 20px; padding: 10px 12px; background: color-mix(in srgb, var(--nd-accent) 8%, transparent); border-radius: 8px; border-left: 3px solid var(--nd-accent); text-align: left;">
                    ...
                    <div style="display: flex; gap: 8px; justify-content: center; flex-wrap: wrap;">
                        <button onclick="window.copySecret('${nsec}'); this.textContent='Copied!'"
                                style="padding: 10px 16px; background: var(--nd-accent); border: none; color: #000; border-radius: 6px; cursor: pointer; font-weight: bold;">
                            📋 Copy
                        </button>
                        <button onclick="window.authUI?.downloadNsecBackup('${nsec}', '${accountLabel}')"
                                style="padding: 10px 16px; background: var(--nd-accent); border: none; color: #fff; border-radius: 6px; cursor: pointer; font-weight: bold;">
                            💾 Download Backup
                        </button>
...
async function saveMoneroAddressToRelays(moneroAddress) {
    ...
    // Get current zap amount from localStorage to preserve it
...
async function saveZapSettingsToRelays(xmrAmount) {
    if (!State.publicKey || !State.getPrivateKeyForSigning()) {
        throw new Error('User not authenticated');
    }

    console.log('💾 Saving zap settings to relays using NIP-78...', { xmr: xmrAmount });
            ...
            <div style="max-width: 800px; margin: 0 auto; padding: 20px;">
                <div style="background: rgba(255, 255, 255, 0.02); border: 1px solid #333; border-radius: 16px; padding: 24px;">
                    ...
                    <div style="text-align: center; color: #666; padding: 40px;">
                        ...
                        <button onclick="loadFollowersPage('${type}', '${pubkey}')" style="background: var(--nd-accent); border: none; border-radius: 8px; padding: 8px 16px; color: white; margin-top: 12px; cursor: pointer;">Retry</button>
                    </div>
                </div>
            </div>
        `;
...
function renderFollowersPage(type, users) {
    ...
    if (users.length === 0) {
        ...
        users.forEach(pubkey => {
            ...
            html += `
                <div onclick="viewUserProfile('${pubkey}')" style="display: flex; align-items: center; gap: 12px; padding: 12px; background: rgba(255, 255, 255, 0.05); border-radius: 8px; cursor: pointer; border: 1px solid transparent; transition: all 0.2s;" onmouseover="this.style.borderColor='var(--nd-accent)'" onmouseout="this.style.borderColor='transparent'">
                    ${profile.picture ?
                        `<img src="${profile.picture}" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover;">` :
                        `<div style="width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); display: flex; align-items: center; justify-content: center; color: white; font-weight: bold;">${name.charAt(0).toUpperCase()}</div>`
                    }
                    <div style="flex: 1;">
...
window.addEventListener('popstate', async (event) => {
    ...
    if (event.state && event.state.page) {
        if (event.state.page === 'thread' && event.state.eventId) {
            ...
            // Click the appropriate feed tab; suggested follows has no tab or rail row,
            // so restore it by calling the feed switch directly
            const feedTab = document.querySelector(`[data-feed="${event.state.feed}"]`);
            if (feedTab) {
                feedTab.click();
            } else if (event.state.feed === 'global' && window.handleFeedTabClick) {
                window.handleFeedTabClick('global', null);
            }
        } else {
            ...
            <div class="modal-content" style="background: #1a1a1a; border: 1px solid #333; border-radius: 16px; padding: 24px; max-width: 500px; width: 90%; max-height: 80vh; overflow-y: auto;">
                ...
                <form id="editProfileForm" onsubmit="saveProfile(event)">
                    ...
                    <div style="display: flex; gap: 12px; margin-top: 24px;">
                        ...
                        <button type="submit" 
                                style="flex: 1; padding: 12px; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; border-radius: 8px; color: #000; font-weight: bold; cursor: pointer; font-size: 16px;">
                            Save Profile
                        </button>
                    </div>
                </form>
...
async function loadSettings() {
    ...
    // Nosdag: wire the settings category sub-nav (idempotent; pane registry in js/nosdag/settings-sections.js).
    // Kept before the profile fetch below so the tabs respond immediately.
    import('./nosdag/settings-sections.js').then((SS) => SS.initSettingsPanes()).catch((e) => console.warn('[nosdag] settings panes failed:', e));
    ...
    // Nosdag: mount the note-history backup/restore section (shell only; stays hidden elsewhere)
    import('./nosdag/history-backup.js').then((HB) => HB.mountBackupSection()).catch((e) => console.warn('[nosdag] backup section failed:', e));
    // Nosdag: mount the timeline import section — pre-Nosdag notes → local IPFS (shell only)
    import('./nosdag/timeline-import.js').then((TI) => TI.mountImportSection()).catch((e) => console.warn('[nosdag] timeline-import section failed:', e));
}
...
async function saveSettings() {
    ...
    try {
        ...
        try {
            if (xmrZapAmount) {
                await saveZapSettingsToRelays(xmrZapAmount || '0.001');
        ...
        // Capture before closing — closeSettingsModal() navigates home, resetting State.currentPage
        const wasOnProfile = State.currentPage === 'profile';
        closeSettingsModal();
        Utils.showNotification('Settings saved successfully!', 'success');
...
async function populateRelayLists() {
    ...
    const renderRow = (relay, accentBg, removeFn, withAnnounce) => {
        ...
        const announceControl = withAnnounce
            ? `<label title="Include this relay in your published NIP-65 announcement. Untick to keep it local-only (won't appear in nevent relay hints other clients generate)."
                      style="display: flex; align-items: center; gap: 4px; font-size: 11px; color: var(--text-secondary); cursor: pointer; white-space: nowrap;">
        ...
        return `
            <div style="display: flex; align-items: center; padding: 8px; background: ${accentBg}; border-radius: 6px; margin-bottom: 6px; gap: 8px;">
                ...
                <button onclick="${removeFn}('${safeRelay}')"
                        style="background: var(--nd-down); border: none; border-radius: 4px; color: var(--nd-on-accent); padding: 4px 8px; font-size: 12px; cursor: pointer;">
                    Remove
                </button>
            </div>
        `;
    ...
    if (readRelaysList) {
        readRelaysList.innerHTML = readRelays
            .map(r => renderRow(r, 'color-mix(in srgb, var(--nd-accent) 10%, transparent)', 'removeReadRelayFromModal', false))
            .join('');
    }
    ...
    if (writeRelaysList) {
        writeRelaysList.innerHTML = writeRelays
            .map(r => renderRow(r, 'color-mix(in srgb, var(--nd-accent) 10%, transparent)', 'removeWriteRelayFromModal', true))
            .join('');
    }
...
async function populateMutedUsersList() {
    ...
    if (!State.mutedUsers || State.mutedUsers.size === 0) {
        mutedUsersList.innerHTML = '<div style="color: var(--text-muted); text-align: center; padding: 20px;">No muted users</div>';
        return;
    }
    ...
    mutedUsersList.innerHTML = mutedPubkeys.map(pubkey => {
        ...
        return `
            <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px; background: color-mix(in srgb, var(--nd-down) 10%, transparent); border-radius: 6px; margin-bottom: 8px;">
                <div style="flex: 1;">
                    <div style="color: var(--text-primary); font-weight: bold;">${displayName}</div>
                    <div style="color: var(--text-muted); font-family: monospace; font-size: 11px;">${pubkey.substring(0, 16)}...</div>
                </div>
                <button onclick="unmutePubkey('${pubkey}')"
                        style="background: var(--nd-ok); border: none; border-radius: 4px; color: var(--text-inverse); padding: 6px 12px; font-size: 12px; cursor: pointer; font-weight: bold;">
                    Unmute
                </button>
            </div>
        `;
...
async function populateMutedHashtagsList() {
    ...
    try {
        ...
        if (tags.length === 0) {
            el.innerHTML = '<div style="color: var(--text-muted); text-align: center; padding: 20px;">No muted hashtags</div>';
            return;
        }
        el.innerHTML = tags.map(t => `
            <div style="display: flex; align-items: center; justify-content: space-between; padding: 8px 10px; background: color-mix(in srgb, var(--nd-accent) 8%, transparent); border-radius: 6px; margin-bottom: 6px;">
                <div style="color: var(--text-primary);">#${escTag(t)}</div>
                <button onclick="removeMutedHashtag('${escTag(t)}', event)"
                        style="background: var(--nd-ok); border: none; border-radius: 4px; color: var(--text-inverse); padding: 5px 10px; font-size: 12px; cursor: pointer; font-weight: bold;">Unmute</button>
            </div>
        `).join('');
    } catch (e) {
...
async function populateMutedWordsList() {
    ...
    try {
        ...
        if (words.length === 0) {
            el.innerHTML = '<div style="color: var(--text-muted); text-align: center; padding: 20px;">No muted words</div>';
            return;
        }
        el.innerHTML = words.map(w => `
            <div style="display: flex; align-items: center; justify-content: space-between; padding: 8px 10px; background: color-mix(in srgb, var(--nd-down) 8%, transparent); border-radius: 6px; margin-bottom: 6px;">
                <div style="color: var(--text-primary);">${escTag(w)}</div>
    ...
    try {
        ...
        modal.innerHTML = `
            <div style="background: #1a1a1a; border-radius: 16px; padding: 24px; max-width: 500px; width: 90%; max-height: 90vh; overflow-y: auto;">
                <h3 style="margin: 0 0 16px; color: var(--nd-accent);">Primary Address</h3>
                ...
                <div style="display: flex; gap: 12px;">
                    <button data-action="copy-primary" style="flex: 1; padding: 12px; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; border-radius: 8px; color: #fff; font-weight: bold; cursor: pointer;">Copy Address</button>
