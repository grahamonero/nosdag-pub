...
// Nosdag: in anonymous (Tor) mode the general relay getters swap to the user's onion-relay
// selection — reads stay end-to-end inside Tor; publishes mirror to clearnet via Tor exits
// unless the user turns the mirror off. Outside the shell every override returns null.
...
export const getActiveRelays = () => {
    ...
    const onion = OnionRelays.generalOverride(State.relays);
...
export const getWriteRelays = () => {
    const clearnet = (userRelayList.write && userRelayList.write.length > 0)
        ? userRelayList.write
        : DEFAULT_RELAYS;
    const onion = OnionRelays.writeOverride(clearnet);
    if (onion) {
        console.log('Anonymous mode: publishing to onion relays' + (onion.length > clearnet.length || onion.some(u => !u.includes('.onion')) ? ' + clearnet mirror' : ' only') + ':', onion);
        return onion;
    }
...
export const getReadRelays = () => {
    const onion = OnionRelays.readOverride();
    if (onion) {
        console.log('Anonymous mode: reading from onion relays only:', onion);
        return onion;
    }
...
export async function getInboxRelays(pubkey) {
    ...
    if (cached && (Date.now() - cached.timestamp) < RELAY_CACHE_TTL) {
        if (cached.read && cached.read.length > 0) {
            return OnionRelays.filterInbox(cached.read.slice(0, MAX_OUTBOX_RELAYS));
        }
    }
    ...
    try {
        ...
        if (relayList && relayList.read && relayList.read.length > 0) {
            ...
            return OnionRelays.filterInbox(relayList.read.slice(0, MAX_OUTBOX_RELAYS));
        }
    } catch (error) {
        console.warn(`Failed to fetch inbox relays for ${pubkey.slice(0, 8)}:`, error.message);
    }

    // Fallback to default relays (strict onion-only mode drops these clearnet targets —
    // the reply still publishes via the author's own write set)
    return OnionRelays.filterInbox(DEFAULT_RELAYS);
}
...
// Get the Nosmero relay URL. Plain-http (local dev on :8080) uses ws; everything
// else — production https AND the desktop shell's app:// origin — uses the secure
// wss endpoint. (Keying on === 'https:' wrongly routed app:// to the dev port,
// which is also blocked as mixed content from the shell's secure context.)
