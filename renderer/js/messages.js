...
export function renderConversations() {
    ...
    conversationsList.innerHTML = sortedConversations.map(([pubkey, conv]) => {
        ...
        const unreadBadge = (conv.unread && conv.unread > 0) ?
            `<div style="position: absolute; top: 12px; right: 12px; width: 10px; height: 10px; background: var(--nd-accent); border-radius: 50%; box-shadow: 0 0 4px var(--nd-accent);"></div>` : '';
        ...
        feed.innerHTML = `
            <div style="padding: 20px; max-width: 800px;">
                <div style="margin-bottom: 30px;">
                    <h2 style="margin-bottom: 20px; color: var(--nd-accent);">🔔 Notifications</h2>
                    ...
                    <!-- Refresh Button -->
                    <button onclick="refreshNotifications()" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #000; padding: 8px 16px; border-radius: 8px; font-weight: bold; cursor: pointer; margin-bottom: 20px;">
                        🔄 Refresh
                    </button>
                </div>
...
export async function fetchNotifications() {
    ...
    try {
        ...
        // Monero tips (kind 9736). Lightning zap receipts (9735) intentionally not ingested — Monero-only.
        ...
        notificationsList.innerHTML = `
            <div style="text-align: center; color: #666; padding: 40px;">
                <div style="margin-bottom: 16px;">
                    <div style="display: inline-block; width: 32px; height: 32px; border: 3px solid #333; border-radius: 50%; border-top-color: var(--nd-accent); animation: spin 1s ease-in-out infinite;"></div>
                </div>
        ...
        return `
            <div class="notification-item" data-type="${notification.type}" data-has-note="${hasOriginalNote}" style="background: var(--card-bg); border: 1px solid var(--border-primary); border-radius: 12px; padding: 16px; margin-bottom: 12px; ${onclickHandler ? 'cursor: pointer;' : ''}"
                ...
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: ${isFollow ? '0' : '12px'};">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                            ...
                            <span style="color: var(--nd-accent); font-weight: bold; cursor: pointer;"
                ...
                ${notification.type === 'tip' && notification.message ? `
                    <!-- Tip message -->
                    <div style="background: color-mix(in srgb, var(--nd-accent) 10%, transparent); padding: 12px; border-radius: 8px; border-left: 3px solid var(--nd-accent); margin-top: 8px;">
