...
export async function hostAccount (pk, onProgress = () => {}) {
  if (!kubo()?.pinRecursive) return { ok: false, error: 'node unavailable' }
  ...
  // 4) media, newest-first, until this account's quota or the global cap is hit
  const media = []
  const seen = new Set()
  ...
  const m = loadHosted()
  m[pk] = { head, cids: pinned, bytes: acctBytes, notes: notes.length, media: mediaPinned, ts: Math.floor(Date.now() / 1000) }
  saveHosted(m)
  return { ok: true, head, notes: notes.length, media: mediaPinned, bytes: acctBytes }
}

/** Stop hosting an account: unpin everything we pinned for them, drop the record. */
...
export async function onFollowChange (pk, nowFollowing) {
  try {
    ...
    if (nowFollowing) { if (autoNewFollows() && !isHosted(pk)) await hostAccount(pk) }
    else if (isHosted(pk)) await unhostAccount(pk)
