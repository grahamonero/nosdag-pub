...
import { signedUrl, requestUrl } from './api-origin.js';
...
export async function uploadToIpfs(file, opts = {}) {
  ...
  // Sign the backend-reconstructed URL; send to the local proxy origin (see api-origin.js).
  const auth = await buildAuthHeader(signedUrl('/api/upload-ipfs'), 'POST');
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open('POST', requestUrl('/api/upload-ipfs'));
...
export async function unpinCid(cid) {
  const path = `/api/upload-ipfs/${encodeURIComponent(cid)}`;
  const auth = await buildAuthHeader(signedUrl(path), 'DELETE');
  const resp = await fetch(requestUrl(path), {
    method: 'DELETE',
...
export async function listPins() {
  const auth = await buildAuthHeader(signedUrl('/api/ipfs-pins'), 'GET');
  const resp = await fetch(requestUrl('/api/ipfs-pins'), { headers: { Authorization: auth } });
  const body = await resp.json().catch(() => ({}));
  if (!resp.ok) {
...
export async function renderIpfsPinsSection(containerEl, viewedPubkey, ownPubkey) {
  ...
  containerEl.innerHTML = `
    <div class="ipfs-pins-section" style="background: rgba(255,255,255,0.02); border: 1px solid #333; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
      <h3 style="margin: 0 0 12px; font-size: 16px; color: var(--text-primary);">📦 IPFS Pins</h3>
  ...
  sect.innerHTML = `
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
      <h3 style="margin: 0; font-size: 16px; color: var(--text-primary);">📦 IPFS Pins</h3>
  ...
  return `
    <div class="ipfs-pin-row" data-cid="${safeCid}" style="background: rgba(255,255,255,0.03); border: 1px solid #2a2a2a; border-radius: 8px; padding: 10px 12px; display: flex; gap: 12px; align-items: center;">
      ...
      <div style="flex: 1; min-width: 0;">
        <div style="color: var(--text-primary); font-size: 13px; font-weight: 500; word-break: break-all; line-height: 1.3;">${safeFilename}</div>
