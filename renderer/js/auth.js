...
export async function getSecurePrivateKey(pin) {
    // Only the encrypted store is served. Raw key material in localStorage
    // ('nostr-private-key') is never returned — that key holds signer markers
    // ('extension'/'amber'/'nsec-app') only.
    ...
    try {
        ...
        // H5: do NOT cache the decrypted key in sessionStorage — it's web-reachable (any injected
        // script / DevTools can read it, even though the sandbox blocks XSS→RCE). The key lives only
        // in-memory (state.js) for the session; reloads (rare in a desktop app) re-prompt the PIN.
        return decryptedKey;
    } catch (error) {
...
export async function completeLoginWithNsec(nsec, displayName = null, options = {}) {
    ...
    try {
        ...
        if (skipPin) {
            // Password users have no PIN and no local encrypted key. H5: don't cache the decrypted
            // key in web-reachable sessionStorage — keep it in-memory only (state.js). A page reload
            // (rare in a desktop app) or restart re-prompts the password, as a restart did before.
            if (DEBUG) console.log('Password login - key held in memory only (no session storage)');
...
export async function loginWithAmber() {
    ...
    try {
        ...
        // The bunker URI carries the connection secret, so it is persisted only when the
        // OS keychain (safeStorage in main) can encrypt it at rest. Without a keychain the
        // URI stays session-only and Amber re-pairs on the next launch.
        try {
            const enc = await window.nosdag?.secrets?.encrypt(bunkerURI);
            if (enc?.data) {
                localStorage.setItem('amber-bunker-uri-enc', enc.data);
            } else {
                showNotification('No OS keychain found — Amber will need to reconnect next launch', 'info');
            }
        } catch (e) {
            console.warn('Could not store bunker URI encrypted:', e?.message);
        }
        ...
        localStorage.removeItem('nostr-private-key');
        localStorage.removeItem('amber-bunker-uri');
        localStorage.removeItem('amber-bunker-uri-enc');
    }
}
...
export async function logout() {
    ...
    // In the Nosdag Electron shell, reload via main (window.nosdag.app.reload) — a
    // renderer-initiated location.reload() over the app:// scheme proved flaky on logout.
    setTimeout(() => {
        if (window.nosdag?.app?.reload) window.nosdag.app.reload();
