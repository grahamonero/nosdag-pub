        ...
        feed.innerHTML = `
            <div style="padding: 20px; max-width: 800px;">
                <div style="margin-bottom: 30px;">
                    <h2 style="margin-bottom: 20px; color: var(--nd-accent);">🔍 Search</h2>
                    ...
                    <div style="display: flex; gap: 12px; margin-bottom: 20px;">
                        ...
                        <button onclick="hideSearchSuggestions(); performSearch()" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #000; padding: 12px 20px; border-radius: 8px; font-weight: bold; cursor: pointer;">
                            Search
                        </button>
                    </div>
                    ...
                    <div style="display: flex; gap: 8px; margin-bottom: 20px; flex-wrap: wrap;">
                        <button id="searchTypeAll" class="search-type-btn active" onclick="setSearchType('all')" 
                                style="padding: 8px 16px; border-radius: 20px; border: 1px solid #333; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); color: #000; cursor: pointer; font-size: 14px;">
                            All
                        </button>
    ...
    if (suggestion) {
        ...
        suggestionHtml = `<div id="spellingSuggestion" style="margin-bottom: 12px; padding: 10px; background: #1a1a2e; border: 1px solid #333; border-radius: 6px;">
               ...
               <a href="#" onclick="searchWithSuggestion('${jsEscape(suggestion.suggested)}'); return false;"
                  style="color: var(--nd-accent); font-weight: bold; text-decoration: underline; cursor: pointer;">
    ...
    searchResults.innerHTML = `
        <div id="searchHeader" style="margin-bottom: 20px; padding: 12px; background: #1a1a1a; border-radius: 8px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <div style="color: var(--nd-accent); font-weight: bold;" id="searchResultsCount">Searching for "${escapeHtml(query)}"...</div>
                <div style="display: none;" id="sortControls">
                    <button id="sortDate" onclick="setSortMode('date')" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #000; padding: 6px 12px; border-radius: 4px; margin-right: 4px; cursor: pointer; font-size: 12px;">By Date</button>
    ...
    if (hasMoreResults) {
        ...
        resultsEl.innerHTML += `
            <div id="loadMoreContainer" style="text-align: center; padding: 20px;">
                <button onclick="loadMoreSearchResults()" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #000; padding: 12px 24px; border-radius: 8px; font-weight: bold; cursor: pointer; font-size: 14px;">
...
function renderSingleResult(post, engagement = { reactions: 0, reposts: 0, replies: 0 }) {
    ...
    if (currentSearchQuery && !currentSearchQuery.startsWith('#') && !currentSearchQuery.startsWith('@')) {
        ...
        highlightedContent = highlightedContent.replace(regex, '<mark style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); color: #000; padding: 2px; border-radius: 2px;">$1</mark>');
    }
    ...
    return `
        <div class="post" data-note-id="${escapeHtml(post.id)}" style="background: #1a1a1a; border-bottom: 1px solid #333; padding: 16px; margin-bottom: 1px;">
            ...
            <div class="post-actions" onclick="event.stopPropagation();" style="display: flex; gap: 16px; align-items: center;">
                ...
                ${moneroAddress ?
                    `<button class="action-btn xmr-zap" onclick="openZapModal('${jsEscapeId}', '${jsEscapeName}', '${jsEscapeMonero}', 'choose', null, '${jsEscapePubkey}')" style="background: none; border: none; color: var(--nd-accent); cursor: pointer; font-size: 14px;" title="Tip with Monero">💰XMR</button>` :
...
export function exportSearchResults() {
    ...
    a.href = url;
    a.download = `nosdag-search-${searchQuery.replace(/[^a-zA-Z0-9]/g, '-')}-${new Date().toISOString().slice(0, 10)}.json`;
