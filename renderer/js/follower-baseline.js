...
// Plain-http (dev on :8080) uses ws; production https AND the app:// shell use wss.
const NOSMERO_RELAY = window.location.protocol === 'http:'
...
/**
 * Last known-good follower count from the LOCAL snapshot only — no relay query, so it
 * answers instantly in any posture. null when no snapshot exists yet (vs a real 0).
 */
export function getLocalFollowerCount() {
