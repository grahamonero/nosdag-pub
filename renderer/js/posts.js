...
// Media attached to the composer — up to MAX_MEDIA_FILES per note, published stacked in
                        ...
                        if (!existingLoadMore && displayedPostCount < State.posts.length) {
                            ...
                            const loadMoreButton = `
                                <div id="loadMoreContainer" style="text-align: center; padding: 20px; border-top: 1px solid var(--border-primary);">
                                    <button onclick="NostrPosts.loadMorePosts()" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #fff; padding: 12px 24px; border-radius: 8px; font-size: 16px; font-weight: bold; cursor: pointer;">
...
export async function renderFeedIncremental() {
    ...
    const loadingIndicator = `
        <div id="feedLoadingIndicator" style="background: linear-gradient(135deg, color-mix(in srgb, var(--nd-accent) 10%, transparent), color-mix(in srgb, var(--nd-accent) 10%, transparent)); padding: 10px; text-align: center; font-size: 14px; color: var(--nd-accent); border-bottom: 1px solid var(--border-primary);">
...
export async function loadStreamingHomeFeed() {
    ...
    try {
        ...
        if (currentHomeFeedResults.length === 0) {
            updateHomeFeedStatus('No notes found. Follow some users to see their notes!');
            const emptyTimelineHtml = `
                <div style="text-align: center; color: #666; padding: 40px;">
                    <p>No notes in your timeline yet!</p>
                    <p style="font-size: 14px; margin-top: 10px;">
                        Follow some users or switch to <a href="#" onclick="loadSearch()" style="color: var(--nd-accent);">Search</a> to discover content.
                    </p>
                    <button onclick="handleFeedTabClick('global', null)" style="margin-top: 18px; background: var(--nd-accent); color: var(--nd-on-accent); border: none; border-radius: var(--nd-radius); padding: 10px 18px; font-size: 14px; font-weight: 700; cursor: pointer;">
                        Browse Suggested Follows
                    </button>
                    <p style="font-size: 12px; margin-top: 8px; color: var(--text-muted);">
                        Notes from well-regarded accounts in your extended network.
                    </p>
                </div>
            `;
            const homeFeedList = document.getElementById('homeFeedList');
            if (homeFeedList) {
                homeFeedList.innerHTML = emptyTimelineHtml;
            } else {
                console.error('homeFeedList element not found - using fallback');
                const feed = document.getElementById('feed');
                if (feed) feed.innerHTML = emptyTimelineHtml;
            }
        } else {
        ...
        const errorHTML = `
            <div class="error" style="color: #ff6666; text-align: center; padding: 40px;">
                ...
                <br><button onclick="reloadHomeFeed()" style="margin-top: 10px; background: var(--nd-accent); border: none; color: #fff; padding: 8px 16px; border-radius: 4px; cursor: pointer;">Retry</button>
            </div>
        `;
        if (homeFeedList) {
...
export async function loadTrendingFeed(forceRefresh = false) {
    ...
    try {
        ...
        const infoHeader = `
            <div style="background: linear-gradient(135deg, color-mix(in srgb, var(--nd-accent) 10%, transparent), color-mix(in srgb, var(--nd-accent) 10%, transparent)); border: 1px solid color-mix(in srgb, var(--nd-accent) 30%, transparent); border-radius: 12px; padding: 16px 20px; margin-bottom: 20px; text-align: center;">
                <div style="color: var(--nd-accent); font-size: 16px; font-weight: bold; margin-bottom: 4px;">
        ...
        const loadMoreButton = hasMorePosts ? `
            <div id="trendingLoadMoreContainer" style="text-align: center; padding: 20px; border-top: 1px solid var(--border-primary);">
                <button onclick="loadMoreTrendingPosts()" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #fff; padding: 12px 24px; border-radius: 8px; font-size: 16px; font-weight: bold; cursor: pointer;">
...
async function loadTrendingFeedForAnonymous(forceRefresh = false) {
    ...
    try {
        ...
        const anonymousBanner = `
            <div style="background: linear-gradient(135deg, color-mix(in srgb, var(--nd-accent) 15%, transparent), color-mix(in srgb, var(--nd-accent) 15%, transparent)); border: 1px solid color-mix(in srgb, var(--nd-accent) 40%, transparent); border-radius: 12px; padding: 20px; margin-bottom: 20px; text-align: center;">
                <div style="color: var(--nd-accent); font-size: 18px; font-weight: bold; margin-bottom: 8px;">
                ...
                <div style="color: #888; font-size: 14px;">
                    <a href="#" onclick="showLoginModal(); return false;" style="color: var(--nd-accent); text-decoration: underline; cursor: pointer;">Login</a> to see your personalized feed
                </div>
            </div>
        `;
...
// Trending notes arrive from /trending-cache.json, which the shell proxies LIVE from the backend.
// Zero-trust: the cache is an external input, so verify every note's signature before it renders or
// enters State.eventCache — otherwise a forged note attributed to any pubkey could appear in the feed
// and, since cached notes are first-class thread roots, seed a thread. Backfill kind on older cache
// files first (their id was hashed as kind 1, #monero-filtered) so genuine notes still verify.
// Falls back to the raw list only when NostrTools isn't loaded yet (matches the L5 path).
function verifyTrendingNotes(notes) {
    const list = Array.isArray(notes) ? notes : [];
    list.forEach(n => { if (n && n.kind === undefined) n.kind = 1; });
    const NT = window.NostrTools;
    if (!NT?.verifyEvent) return list;
    const verified = list.filter(n => { try { return NT.verifyEvent(n); } catch { return false; } });
    const dropped = list.length - verified.length;
    if (dropped > 0) console.warn(`⚠️ Trending cache: dropped ${dropped} note(s) that failed signature verification`);
    return verified;
}
...
async function renderCachedTrendingFeed(cache) {
    ...
    // Drop any operator-forged notes before they render or get cached (kind is backfilled inside).
    cache.notes = verifyTrendingNotes(cache.notes);
    ...
    // Cache the verified notes in eventCache — embedded-nevent resolution and thread roots read here.
    ...
    const anonymousBanner = `
        <div style="background: linear-gradient(135deg, color-mix(in srgb, var(--nd-accent) 15%, transparent), color-mix(in srgb, var(--nd-accent) 15%, transparent)); border: 1px solid color-mix(in srgb, var(--nd-accent) 40%, transparent); border-radius: 12px; padding: 20px; margin-bottom: 20px; text-align: center;">
            ...
            <div style="color: #888; font-size: 13px; margin-bottom: 12px;">
                Last updated ${timeAgo} • <a href="#" onclick="refreshTrendingFeed(); return false;" style="color: var(--nd-accent); text-decoration: underline; cursor: pointer;">Refresh now</a>
            </div>
            <div style="color: #888; font-size: 14px;">
                <a href="#" onclick="showLoginModal(); return false;" style="color: var(--nd-accent); text-decoration: underline; cursor: pointer;">Login</a> to see your personalized feed
            </div>
    ...
    // Resolve embedded nostr:nevent/note quotes. The anonymous trending render was the only
    // feed path missing this call, so embeds stayed stuck on "Loading note…" for logged-out
    // users (worked in thread view + logged-in, which do call it).
    await Utils.processEmbeddedNotes('homeFeedList');
    ...
    const infoHeader = `
        <div style="background: linear-gradient(135deg, color-mix(in srgb, var(--nd-accent) 10%, transparent), color-mix(in srgb, var(--nd-accent) 10%, transparent)); border: 1px solid color-mix(in srgb, var(--nd-accent) 30%, transparent); border-radius: 12px; padding: 16px 20px; margin-bottom: 20px; text-align: center;">
            ...
            <div style="color: #888; font-size: 13px;">
                Last updated ${timeAgo} • <a href="#" onclick="refreshTrendingFeedLoggedIn(); return false;" style="color: var(--nd-accent); text-decoration: underline; cursor: pointer;">Refresh now</a>
            </div>
        </div>
    `;
...
async function loadTrendingFeedForNewUser() {
    ...
    try {
        ...
        // Drop any operator-forged notes (cache branch) before they render or get cached.
        trendingNotes = verifyTrendingNotes(trendingNotes);
        ...
        // Cache the verified notes in eventCache
        ...
        const starterPackButtons = Object.entries(STARTER_PACKS)
            ...
            .map(([key, pack]) => `
                <button onclick="followStarterPack('${key}')"
                        ...
                        onmouseover="this.style.borderColor='var(--nd-accent)'; this.style.background='#2a2a2a';"
        ...
        const onboardingBanner = `
            <div style="background: linear-gradient(135deg, color-mix(in srgb, var(--nd-accent) 15%, transparent), color-mix(in srgb, var(--nd-accent) 15%, transparent)); border: 1px solid color-mix(in srgb, var(--nd-accent) 40%, transparent); border-radius: 12px; padding: 24px; margin-bottom: 20px;">
                <div style="text-align: center; margin-bottom: 20px;">
                    <div style="color: var(--nd-accent); font-size: 22px; font-weight: bold; margin-bottom: 8px;">
                        Welcome to Nosdag!
                    </div>
                ...
                <div style="text-align: center;">
                    <a href="#" onclick="loadSearch(); return false;" style="color: var(--nd-accent); text-decoration: underline; cursor: pointer; font-size: 14px;">
...
export async function loadWebOfTrustFeed() {
    ...
    feed.innerHTML = `
        <div style="padding: 20px; text-align: center; color: #999;">
            <div class="spinner" style="width: 40px; height: 40px; border: 3px solid rgba(255, 255, 255, 0.1); border-top-color: var(--nd-accent); border-radius: 50%; animation: spin 0.8s linear infinite; margin: 0 auto 16px;"></div>
        ...
        if (postsToDisplay.length === 0) {
            ...
            const loadMoreButton = `
                <div id="webOfTrustLoadMoreContainer" style="text-align: center; padding: 20px; border-top: 1px solid var(--border-primary);">
                    <button onclick="loadMoreWebOfTrustPosts()" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #fff; padding: 12px 24px; border-radius: 8px; font-size: 16px; font-weight: bold; cursor: pointer;">
                        Load More Posts
                    </button>
                </div>
            `;
    ...
    try {
        ...
        if (loadMoreContainer) {
            loadMoreContainer.innerHTML = `
                <div style="padding: 20px; text-align: center; color: #999;">
                    <div class="spinner" style="width: 30px; height: 30px; border: 2px solid rgba(255, 255, 255, 0.1); border-top-color: var(--nd-accent); border-radius: 50%; animation: spin 0.8s linear infinite; margin: 0 auto;"></div>
                </div>
            `;
        }
        ...
        if (followYs.size === 0) {
            ...
            if (loadMoreContainer) {
                loadMoreContainer.innerHTML = `
                    <p style="color: #666; text-align: center; padding: 20px;">No users found in this batch. <a href="#" onclick="loadMoreWebOfTrustPosts(); return false;" style="color: var(--nd-accent);">Try loading more</a></p>
                `;
            }
            return;
        }
...
async function renderFeedFromCache() {
    ...
    try {
        ...
        const [cachedProfiles, cachedEventsRaw] = await Promise.all([
            FeedCache.getCachedProfiles(followingArray),
            FeedCache.getCachedEvents(followingArray, 200)
        ]);
        // L5: events came from local IndexedDB — re-verify signatures before trusting/rendering them,
        // so a tampered local cache can't inject forged notes. (Profiles are display-only metadata.)
        const NT = window.NostrTools;
        const cachedEvents = NT?.verifyEvent
            ? cachedEventsRaw.filter(ev => { try { return NT.verifyEvent(ev); } catch { return false; } })
            : cachedEventsRaw;
    ...
    feed.innerHTML = `
        <div id="homeFeedHeader" style="margin-bottom: 20px; padding: 12px; background: var(--bg-tertiary); border-radius: 8px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <div style="color: var(--nd-accent); font-weight: bold;" id="homeFeedCount">Loading your timeline...</div>
                <div style="display: none;" id="homeFeedSortControls">
                    <button id="homeSortStream" onclick="setHomeFeedSortMode('stream')" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #000; padding: 6px 12px; border-radius: 4px; margin-right: 4px; cursor: pointer; font-size: 12px;">As Found</button>
        ...
        <div id="loadMoreContainer" style="display: none; text-align: center; margin: 20px 0; padding: 20px;">
            <button onclick="NostrPosts.loadMorePosts()" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #fff; padding: 12px 32px; border-radius: 8px; font-size: 16px; font-weight: bold; cursor: pointer;">
                Load More Notes
            </button>
        </div>
    `;
            ...
            <div class="parent-post" onclick="openThreadView('${parentPost.id}')" style="cursor: pointer; margin-bottom: 8px;">
                <div class="post-header" style="font-size: 14px;">
                    ${parentAuthor.picture ?
                        `<img class="avatar" src="${parentAuthor.picture}" alt="${parentAuthor.name}" onclick="viewUserProfilePage('${parentPost.pubkey}'); event.stopPropagation();" style="width: 24px; height: 24px; border-radius: 50%; object-fit: cover; cursor: pointer;" />` :
                        `<div class="avatar" onclick="viewUserProfilePage('${parentPost.pubkey}'); event.stopPropagation();" style="width: 24px; height: 24px; border-radius: 50%; background: var(--bg-hover); display: flex; align-items: center; justify-content: center; font-size: 12px; cursor: pointer;">${parentAuthor.name ? parentAuthor.name.charAt(0).toUpperCase() : '?'}</div>`
                    }
                    <div class="post-info">
                        <span class="username" data-pubkey="${parentPost.pubkey}" onclick="viewUserProfilePage('${parentPost.pubkey}'); event.stopPropagation();" style="font-size: 14px; cursor: pointer;">${parentAuthor.name}</span>
...
export async function renderFeed(loadMore = false) {
    ...
    const postsHtml = postsToRender.map((post, index) => {
        ...
        return `
            <div class="post" data-post-id="${post.id}" data-pubkey="${post.pubkey}" data-client="${(post.tags || []).find(t => t[0] === 'client')?.[1] || ''}">
                ${parentHtml}
                ...
                <div class="post-actions" onclick="event.stopPropagation();">
                    ...
                    ${'' /* Bitcoin Lightning zaps removed — Nosdag is Monero-only */}
                    ${moneroAddress ?
                ...
                <div style="padding: 8px 12px; margin-top: 8px; background: linear-gradient(135deg, color-mix(in srgb, var(--nd-accent) 10%, transparent), color-mix(in srgb, var(--nd-accent) 10%, transparent)); border-radius: 8px; border: 1px solid color-mix(in srgb, var(--nd-accent) 20%, transparent);">
                    ...
                    <div style="display: flex; align-items: center; justify-content: space-between; font-size: 13px;">
                        <div style="color: var(--nd-accent); font-weight: bold;">
                    ...
                    <div style="margin-top: 6px;">
                        <button onclick="showDisclosedTipDetails('${post.id}', event)" style="background: none; border: 1px solid var(--nd-accent); color: var(--nd-accent); padding: 4px 8px; border-radius: 4px; font-size: 11px; cursor: pointer;">View Details</button>
                    </div>
                    ` : ''}
                </div>
                ` : ''}
...
export async function likePost(postId) {
    ...
    try {
        if (wasLiked) {
            ...
            State.pool.publish(Relays.getWriteRelays(), signedEvent);
            // Nosdag reciprocal channel (§6.3): liking someone opens a short channel so their
            // replies in this thread reach your wall directly (within TTL) instead of Requests.
            import('./nosdag/reciprocal-channel.js')
                .then(R => R.openFromNote(post))
                .catch(e => console.warn('[nosdag] reciprocal open (like) failed:', e));
...
export function setRepostType(type) {
    ...
    if (type === 'quick') {
        ...
        quickBtn.style.background = 'linear-gradient(135deg, var(--nd-accent), var(--nd-accent))';
        ...
        commentBtn.style.background = 'linear-gradient(135deg, var(--nd-accent), var(--nd-accent))';
...
export async function sendReply(replyToId) {
    ...
    try {
        ...
        if (currentMediaFiles.length) {
            ...
            try {
                mediaUrls = await uploadMediaToBlossom();
            } catch (error) {
        ...
        // Nosdag DAG: replies chain into the author's note DAG too — add the prev tag before signing.
        const NosdagDag = await import('./nosdag/dag-publish.js');
        const nosdagPrev = NosdagDag.getLocalHead(State.publicKey);
        if (nosdagPrev) eventTemplate.tags.push(['prev', nosdagPrev]);
        // Nosdag: derive NIP-92 imeta tags from any ipfs:// media refs in content (design §13.1) before signing.
        try { const Media = await import('./nosdag/media.js'); Media.addImetaTags(eventTemplate); } catch (e) { console.warn('[nosdag] imeta derive failed:', e); }
        // Nosdag Phase 5 · Slice 5 — publish media as a gateway URL so any Nostr client renders it (Nosdag re-points it to the local node on render).
        try { const Interop = await import('./nosdag/interop-publish.js'); Interop.applyInterop(eventTemplate); } catch (e) { console.warn('[nosdag] interop rewrite failed:', e); }
        ...
        // Nosdag DAG: store the reply in IPFS + advance the head (best-effort, non-blocking)
        NosdagDag.publishToDag({
            signedEvent, prevCid: nosdagPrev, pubkey: State.publicKey,
            signEvent: Utils.signEvent, pool: State.pool, writeRelays: myWriteRelays
        }).catch((e) => console.warn('[nosdag] reply DAG publish failed (reply still posted):', e));

        // Nosdag reciprocal channel (§6.3): replying opens a channel so the recipient's replies
        // back in this thread reach your wall directly (within TTL) instead of your Requests queue.
        import('./nosdag/reciprocal-channel.js')
            .then(R => R.openFromNote(originalPost, [signedEvent.id]))
            .catch(e => console.warn('[nosdag] reciprocal open (reply) failed:', e));
            ...
            const sub = State.pool.subscribeMany([nosmeroRelay], [
                ...
                onevent(event) {
                    try {
                        ...
                        const confirmationsTag = event.tags.find(tag => tag[0] === 'confirmations');
                        const proofHashTag = event.tags.find(tag => tag[0] === 'proof_hash');
                        ...
                        if (amount > 0) {
                            const tipData = {
                                ...
                                confirmations: confirmationsTag ? parseInt(confirmationsTag[1], 10) : null,
                                proofHash: proofHashTag ? proofHashTag[1] : null
                            };
...
export async function fetchProfiles(pubkeys) {
    ...
    try {
        ...
        // NOTE: nostr.band removed Dec 28, 2025 - SSL cert expired Dec 22
        // In Tor posture the clearnet list can't resolve — the onion read set replaces it.
        let majorRelays = [
            'wss://relay.damus.io',
            'wss://nos.lol',
            'wss://relay.primal.net'
        ...
        try {
            ...
            majorRelays = OR.readOverride?.() || majorRelays;
...
export async function renderSinglePost(post, context = 'feed', engagementData = null, parentPostsMap = null, repostContext = null) {
    try {
        ...
        return `
            ${repostHtml}
            <div class="post" data-post-id="${post.id}" data-pubkey="${post.pubkey}" data-client="${(post.tags || []).find(t => t[0] === 'client')?.[1] || ''}" ${repostContext ? `data-repost-id="${repostContext.repostId}"` : ''}>
...
export async function sendPost() {
    ...
    try {
        ...
        if (currentMediaFiles.length) {
            ...
            try {
                const mediaUrls = await uploadMediaToBlossom();
                if (!mediaUrls?.length) {
        ...
        // Nosdag DAG: chain this post to the author's previous one. Read the local head and add
        // the prev STRING tag BEFORE signing (so the signed event matches the IPLD link main builds).
        const NosdagDag = await import('./nosdag/dag-publish.js');
        const nosdagPrev = NosdagDag.getLocalHead(State.publicKey);
        if (nosdagPrev) event.tags.push(['prev', nosdagPrev]);
        // Nosdag: derive NIP-92 imeta tags from any ipfs:// media refs in content (design §13.1) before signing.
        try { const Media = await import('./nosdag/media.js'); Media.addImetaTags(event); } catch (e) { console.warn('[nosdag] imeta derive failed:', e); }
        // Nosdag Phase 5 · Slice 5 — publish media as a gateway URL so any Nostr client renders it (Nosdag re-points it to the local node on render).
        try { const Interop = await import('./nosdag/interop-publish.js'); Interop.applyInterop(event); } catch (e) { console.warn('[nosdag] interop rewrite failed:', e); }
        ...
        // Nosdag DAG: also store this post in IPFS as a dag-cbor envelope + announce
        // the nosdag:head pointer. Best-effort and non-blocking — an IPFS hiccup must never block
        // a normal Nostr post. (The relay publish above is the Text Billboard; this is the payload.)
        NosdagDag.publishToDag({
            signedEvent,
            prevCid: nosdagPrev,
            pubkey: State.publicKey,
            signEvent: Utils.signEvent,
            pool: State.pool,
            writeRelays
        }).catch((e) => console.warn('[nosdag] DAG publish failed (note still posted to relays):', e));
...
function ensureCursorVisibility() {
    ...
    if (textarea) {
        ...
        textarea.addEventListener('input', function() {
            ...
            this.style.caretColor = 'var(--nd-accent)';
        });
...
// Handle media file selection — the input is `multiple`; each pick APPENDS to the current
// attachments (so the user can add files across several picks) up to MAX_MEDIA_FILES.
export function handleMediaUpload(input, context) {
    ...
    for (const file of files) {
        ...
        if (currentMediaFiles.length >= MAX_MEDIA_FILES) {
            Utils.showNotification(`Up to ${MAX_MEDIA_FILES} media files per note — the rest were skipped`, 'error');
            break;
        }
...
// Show the attachment list — one row + thumbnail per file, stacked like the published note,
// each with its own remove button.
function showMediaPreview(context) {
    const previewId = context === 'reply' ? 'replyMediaPreview' : 'composeMediaPreview';
    ...
    preview.innerHTML = currentMediaFiles.map((file, i) => {
        const fileSize = (file.size / 1024 / 1024).toFixed(2) + ' MB';
        const fileType = file.type.startsWith('image/') ? '🖼️' : '🎥';
        return `
        <div class="media-info">
            <span>${fileType} ${Utils.escapeHtml(file.name)} (${fileSize})</span>
            <button class="remove-media" onclick="removeMedia('${context}', ${i})">Remove</button>
        </div>
        <div id="${context}MediaContent${i}"></div>`;
    }).join('');
...
// Remove one attachment by index, or all of them when no index is given (the post-publish
// cleanup path calls it that way).
export function removeMedia(context, index) {
    if (typeof index === 'number') currentMediaFiles.splice(index, 1);
    else currentMediaFiles = [];

    const inputId = context === 'reply' ? 'replyMediaInput' : 'composeMediaInput';
...
// Upload every attachment in order. Returns an array of refs (ipfs:// in the shell, host URLs
// on the web) to append to the note — one per line, so they render stacked. Throws on the
// first failure: a note should carry ALL its media or not publish.
async function uploadMediaToBlossom() {
    if (!currentMediaFiles.length) return null;

    // Nosdag: in the shell, media goes ONLY to the user's own local IPFS node, referenced as
    // ipfs://<CID>. IPFS-or-fail by design — NO silent fallback to centralized HTTP hosts: that
    // would defeat the point (sovereign media) and hide a node outage behind a third-party upload.
    // If the local node isn't ready yet, addFileToIpfs throws a clear message and the post is
    // blocked (the caller shows it as "Media upload failed: …").
    ...
    if (Media.inNosdagShell()) {
        ...
        for (const file of currentMediaFiles) {
            ...
            console.log('[nosdag] 📦 media in IPFS:', ref);
            refs.push(ref);
        }
        return refs;
    }

    // Outside the shell (plain web frontend, no local node) the legacy HTTP hosts are the only option.
    const urls = [];
    for (const original of currentMediaFiles) {
        const fileToUpload = await Utils.stripImageMetadata(original);
        console.log('Starting media upload for file:', fileToUpload.name, 'Size:', fileToUpload.size, 'Type:', fileToUpload.type, '(metadata stripped:', fileToUpload !== original, ')');
        ...
        for (const provider of mediaProviders) {
            try {
                ...
                uploaded = await provider.upload(fileToUpload);
...
export async function showDisclosedTipDetails(postId, event) {
    ...
    const tipsListHtml = sortedTips.map(tip => {
        ...
        const amountColor = isMuted ? '#999' : (tip.verified ? '#10B981' : 'var(--nd-accent)');
            ...
            <div style="padding: 12px; border-bottom: 1px solid var(--border-primary); background: ${bgColor}; ${isMuted ? 'opacity: 0.6;' : ''}">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px;">
                    <div style="display: flex; align-items: center; gap: 8px; flex: 1; flex-wrap: wrap;">
                        ...
                        ${tip.verified ? `
                            <span style="background: rgba(16, 185, 129, 0.2); border: 1px solid #10B981; color: #10B981; padding: 2px 6px; border-radius: 3px; font-size: 10px; font-weight: bold;" title="Verified by Nosdag">
                                ✓ VERIFIED
                            </span>
                        ` : ''}
                        ${isMuted ? `
                ...
                ${tip.verified ? `
                    <div style="margin-top: 6px; font-size: 11px; color: #10B981; font-style: italic;">
                        🔐 Verified on-chain${tip.confirmations != null ? ` · ${tip.confirmations} confirmation${tip.confirmations === 1 ? '' : 's'}` : ''}${tip.verifiedBy ? ` · ${Utils.escapeHtml(tip.verifiedBy)}` : ''}
                    </div>
                ` : ''}
        ...
        <div id="disclosedTipsModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.85); display: flex; align-items: center; justify-content: center; z-index: 10000;" onclick="closeDisclosedTipsModal()">
            <div style="background: var(--bg-tertiary); border-radius: 16px; max-width: 600px; width: 90%; max-height: 80vh; overflow: hidden; box-shadow: 0 8px 32px rgba(0,0,0,0.5); border: 1px solid var(--border-primary);" onclick="event.stopPropagation()">
                <div style="padding: 20px; border-bottom: 1px solid var(--border-primary); display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <h2 style="margin: 0; color: var(--nd-accent);">💰 Tips for this Note</h2>
                        <div style="font-size: 14px; color: #999; margin-top: 8px;">
                            ...
                            ${disclosedCount > 0 ? `<div style="color: var(--nd-accent);">💰 Disclosed: ${disclosedTotal.toFixed(4)} XMR (${disclosedCount})</div>` : ''}
...
async function updateWidgetReceivedTips() {
    ...
    const tipsHtml = allReceivedTips.map(tip => {
        ...
        const timeAgo = Utils.formatTime(tip.timestamp);
        const badge = tip.verified ? '<span style="color: #10B981;">✓</span>' : '<span style="color: var(--nd-accent);">💰</span>';
        ...
        return `
            <div ${clickHandler} style="padding: 6px 0; border-bottom: 1px solid color-mix(in srgb, var(--nd-accent) 10%, transparent); ${cursorStyle}">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <div style="font-size: 11px; color: var(--nd-accent); font-weight: bold; margin-left: 8px;">
...
export async function sendReplyDirect(replyToId, content) {
    ...
    let originalPost = State.posts.find(p => p.id === replyToId) || State.eventCache[replyToId];
    if (!originalPost) {
        // Not in feed state or the event cache (seen replying from a thread surface) —
        // recover by fetching the note from the read relays instead of dead-ending.
        console.warn('[Posts] Reply target not cached, fetching from relays:', replyToId);
        try {
            const events = await Promise.race([
                State.pool.querySync(Relays.getActiveRelays(), { ids: [replyToId] }),
                new Promise((resolve) => setTimeout(() => resolve([]), 4000))
            ]);
            const fetched = (events || []).find(e => e?.id === replyToId);
            if (fetched && window.NostrTools?.verifyEvent?.(fetched)) {
                State.eventCache[replyToId] = fetched;
                originalPost = fetched;
            }
        } catch (e) {
            console.warn('[Posts] Reply target relay fetch failed:', e?.message);
        }
    }
    if (!originalPost) {
        console.error('[Posts] Reply target not found in state, cache, or relays:', replyToId);
...
export async function sendPostDirect(content) {
    ...
    try {
        ...
        // Nosdag DAG: chain this post to the author's previous one — add the prev STRING tag
        // BEFORE signing so the signed event matches the IPLD link main builds.
        const NosdagDag = await import('./nosdag/dag-publish.js');
        const nosdagPrev = NosdagDag.getLocalHead(State.publicKey);
        if (nosdagPrev) eventTemplate.tags.push(['prev', nosdagPrev]);
        // Nosdag: derive NIP-92 imeta tags from any ipfs:// media refs in content (design §13.1) before signing.
        ...
        // Nosdag DAG: also store the post in IPFS as a dag-cbor envelope + announce
        // the nosdag:head pointer. Best-effort/non-blocking — an IPFS hiccup never blocks the post.
        NosdagDag.publishToDag({
            signedEvent, prevCid: nosdagPrev, pubkey: State.publicKey,
            signEvent: Utils.signEvent, pool: State.pool, writeRelays
        }).catch((e) => console.warn('[nosdag] DAG publish failed (note still posted to relays):', e));
...
function renderTipActivityWidget() {
    ...
    feed.innerHTML = `
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="padding: 20px; background: color-mix(in srgb, var(--nd-accent) 5%, transparent); border: 1px solid color-mix(in srgb, var(--nd-accent) 20%, transparent); border-radius: 12px;">
                <div style="margin-bottom: 12px;">
                    <div style="font-weight: bold; font-size: 18px; color: var(--nd-accent);">
                        💰 Public Tips
                    </div>
                </div>
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
                    ...
                    <div style="font-size: 14px; color: var(--nd-accent); font-weight: bold;">${widgetNetworkStats.totalXMR.toFixed(5)} XMR</div>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
                    <div style="font-size: 14px; color: var(--text-secondary);">Tips:</div>
                    <div style="font-size: 14px; color: var(--nd-accent); font-weight: bold;">${widgetNetworkStats.count}</div>
                </div>

                <!-- Personal Stats (shown when logged in) -->
                ${State.publicKey ? `
                <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid color-mix(in srgb, var(--nd-accent) 20%, transparent);">
                    <div onclick="toggleWidgetReceivedTips()" style="cursor: pointer; font-size: 13px; color: #999; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <span id="widgetReceivedArrow" style="color: var(--nd-accent); font-size: 16px;">▼</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
                    ...
                    <div style="display: flex; justify-content: space-between;">
                        ...
                        <div style="font-size: 14px; color: var(--nd-accent); font-weight: bold;">${widgetPersonalStats.disclosed.totalXMR.toFixed(5)} XMR</div>
                    </div>

                    <!-- Expandable received tips list -->
                    <div id="widgetReceivedExpanded" style="display: none; margin-top: 12px; padding-top: 12px; border-top: 1px solid color-mix(in srgb, var(--nd-accent) 15%, transparent);">
                ...
                <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid color-mix(in srgb, var(--nd-accent) 20%, transparent);">
                    <div onclick="toggleWidgetSentTips()" style="cursor: pointer; font-size: 13px; color: #999; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center;">
                        <span>You Sent</span>
                        <span id="widgetSentArrow" style="color: var(--nd-accent); font-size: 16px;">▼</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
                        <div style="font-size: 14px; color: var(--text-secondary);">Total:</div>
                        <div style="font-size: 14px; color: var(--nd-accent); font-weight: bold;">${widgetSentStats.totalXMR.toFixed(5)} XMR</div>
                    </div>
                    <div style="display: flex; justify-content: space-between;">
                        <div style="font-size: 14px; color: var(--text-secondary);">Tips:</div>
                        <div style="font-size: 14px; color: var(--nd-accent); font-weight: bold;">${widgetSentStats.count}</div>
                    </div>

                    <!-- Expandable sent tips list -->
                    <div id="widgetSentExpanded" style="display: none; margin-top: 12px; padding-top: 12px; border-top: 1px solid color-mix(in srgb, var(--nd-accent) 15%, transparent);">
...
async function showMoreTrendingAll() {
    ...
    const loadMoreButton = hasMore ? `
        <div id="loadMoreContainer" style="text-align: center; padding: 20px;">
            <button onclick="window.loadMoreTrendingAll()" style="padding: 12px 24px; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 500;">
                Load More
            </button>
        </div>
    ` : '';
