...
export async function handleSignup(e) {
  ...
  try {
    ...
    if (existingNsec) {
      ...
      const { nsecEncode, npubEncode } = window.NostrTools.nip19;
...
export function downloadNsecBackup(nsec, accountLabel) {
  ...
  const filename = `nosdag-backup-${safeLabel}-${date}.txt`;

  const contents = `NOSDAG ACCOUNT BACKUP
======================

This file contains your Nostr private key (nsec). It is the ONLY way to
recover your Nosdag account if you forget your password — Nosdag cannot
...
export function updatePasswordStrength(password) {
  ...
  const labels = [
    'Weak — easy to crack if someone gets your device',
    'Fair — add more length or characters',
    'Good — solid protection on this device',
    'Strong — excellent protection on this device'
  ];
