 ...
 *
 * The noble crypto is vendored locally (renderer/lib/noble/) and byte-identical to the npm packages,
 * so the nsec-decryption path has no runtime network dependency — a compromised CDN can't swap in code
 * that steals the key during login. Interoperable with other NIP-49 implementations (noStrudel, etc.)
 */
...
/**
 * Load the locally-vendored noble crypto (renderer/lib/noble/), lazily and once.
 */
async function loadNobleLibraries() {
  if (scrypt && xchacha20poly1305) return;

  try {
    // scrypt from @noble/hashes (vendored)
    const hashesModule = await import('/lib/noble/scrypt.js');
    scrypt = hashesModule.scrypt;

    // XChaCha20-Poly1305 from @noble/ciphers (vendored)
...
export function validatePassword(password) {
  ...
  // Local-only accounts: the password encrypts the key on THIS device, so we don't hard-enforce a
  // strength floor — any non-empty password is accepted and the UI strength meter warns on weak ones
  // (the nsec backup is the real recovery). Just guard against an empty/oversized/malformed value.
  if (password.length > 128) {
