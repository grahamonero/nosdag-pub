...
export function showNotification(message, type = 'success') {
    ...
    notification.style.cssText = `
        ...
        background: ${type === 'success' ? 'linear-gradient(135deg, #4CAF50, #45a049)' : 
                     ...
                     'linear-gradient(135deg, var(--nd-accent), var(--nd-accent))'};
        color: white;
        padding: 16px 24px;
        border-radius: 8px;
...
export function renderLoginRequired(container, message = 'Please login to access this feature') {
    ...
    container.innerHTML = `
        <div style="padding: 40px; text-align: center; max-width: 500px; margin: 0 auto;">
            <h2 style="color: var(--nd-accent); margin-bottom: 30px;">Login Required</h2>
...
function parseMarkdownContent(content, skipEmbeddedNotes = false) {
    ...
    // NOTE: the placeholder MUST be markdown-inert — a `__…__` token gets eaten by marked as
    // bold (→ <strong>…</strong>), which broke the restore regex and leaked the literal token
    // for any note mixing markdown with a nostr mention. Pure alphanumerics + an explicit
    // terminator are safe and unambiguous.
    let processed = content.replace(/(nostr:)?(npub1[a-z0-9]{58}|nprofile1[a-z0-9]+|note1[a-z0-9]{58}|nevent1[a-z0-9]+|naddr1[a-z0-9]+)/gi, (match) => {
        mentions.push(match);
        return `NOSTRMENTIONSLOT${mentionIndex++}ENDSLOT`;
    });
    ...
    parsed = parsed.replace(/NOSTRMENTIONSLOT(\d+)ENDSLOT/g, (match, index) => {
...
export function parseContent(content, skipEmbeddedNotes = false) {
    // Nosdag: render IPFS media from the LOCAL node. A note may carry media two ways — a portable
    // ipfs://<CID> ref, or (Slice 5 smart-publish) a https://dweb.link/ipfs/<CID> gateway URL baked
    // in so vanilla Nostr clients render it too. Inside the shell, re-point BOTH at the local
    // gateway so Nosdag serves the CID P2P (mirror-fetch falls back to a public gateway if the local
    // node can't get the block); the CID is identical either way, so the image/video regexes and the
    // markdown path display them, Bitswap-backed. No-op outside the shell. The dweb.link host must
    // match interop-publish.js GATEWAY.
    if (typeof content === 'string' && window.nosdag?.kubo?.gateway) {
        const gw = window.nosdag.kubo.gateway;
        if (content.indexOf('ipfs://') !== -1) content = content.replace(/ipfs:\/\//gi, gw);
        if (content.indexOf('dweb.link/ipfs/') !== -1) content = content.replace(/https?:\/\/dweb\.link\/ipfs\//gi, gw);
    }
...
export async function processEmbeddedNotes(containerId) {
    ...
    // Nosdag: tag this user's own notes that live in their IPFS DAG (best-effort, non-blocking)
    import('./nosdag/feed-marks.js').then(FM => FM.markIpfsNotes(containerId)).catch(() => {});
    // Nosdag: tag conversation roots you follow (Phase 5 · Slice 4 thread-follow chip)
    import('./nosdag/consumption-tiers.js').then(CT => CT.markFollowedThreads(containerId)).catch(() => {});
    // Nosdag: install the mirror-fetch + "media resting" tombstone layer (self-installs once)
    import('./nosdag/mirror-fetch.js').catch(() => {});
        ...
        <div class="embedded-note-content" data-action="open-thread" data-eventid="${escapeHtml(event.id)}">
            <div style="display: flex !important; flex-direction: row !important; align-items: center !important; gap: 8px !important; margin-bottom: 6px;">
                ${hasValidPicture ?
                    ...
                    `<div style="width: 20px !important; height: 20px !important; min-width: 20px !important; min-height: 20px !important; border-radius: 50% !important; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)) !important; display: inline-flex !important; align-items: center !important; justify-content: center !important; color: white !important; font-size: 10px !important; font-weight: bold !important; flex-shrink: 0 !important;">${escapeHtml(authorName.charAt(0).toUpperCase())}</div>`
                }
...
export function initGlobalEventDelegation() {
    ...
    // Capture phase: these elements live INSIDE .post-content, whose inline onclick opens the
    // thread. A bubble-phase listener runs after that handler, so clicking a mention opened the
    // thread and the profile raced it. Capturing + stopImmediatePropagation makes the intended
    // action the only one that fires.
