...
import { fetchXMRPrice, formatUSD, openWalletModal } from '../wallet-modal.js';
...
export function showLoginModal() {
    ...
    if (modal) {
        ...
        if (header) header.textContent = 'Sign In to Nosdag';
    }
}
...
export function showCreateAccountModal() {
    ...
    if (modal) {
        ...
        document.getElementById('newUserSection')?.classList.remove('hidden');
        const header = document.getElementById('loginModalHeader');
        if (header) header.textContent = 'Join Nosdag';
        document.getElementById('displayNameInput')?.focus();
    }
}
    ...
    if (feed) {
        feed.innerHTML = `
            <div style="padding: 40px; text-align: center; max-width: 500px; margin: 0 auto;">
                <h2 style="color: var(--nd-accent); margin-bottom: 30px;">Login with Private Key</h2>
                ...
                <div style="margin-bottom: 24px; padding: 16px; background: color-mix(in srgb, var(--nd-accent) 10%, transparent); border-radius: 8px; border-left: 4px solid var(--nd-accent); max-width: 400px; margin: 0 auto 24px auto;">
                    <div style="color: var(--nd-accent); font-weight: bold; font-size: 14px; margin-bottom: 8px;">⚠️ Critical Security Warning</div>
                    <div style="color: #ccc; font-size: 13px; line-height: 1.6; text-align: left;">
                        <strong style="color: var(--nd-accent);">Anyone with your nsec can control your account permanently.</strong> There is no password reset or recovery - if your nsec is exposed or lost, your identity is compromised forever.
                    </div>
                </div>
                ...
                <div style="font-size: 12px; color: #666; text-align: left; max-width: 400px; margin: 0 auto;">
                    ...
                    <ul style="text-align: left; margin: 10px 0;">
                        ...
                        <li>Consider Amber remote signing (NIP-46) to keep your key off this machine</li>
                    </ul>
                </div>
            </div>
        `;
    ...
    if (feed) {
        feed.innerHTML = `
            <div style="padding: 40px; text-align: center; max-width: 600px; margin: 0 auto;">
                <h2 style="color: var(--nd-accent); margin-bottom: 30px;">📱 Login with Amber</h2>
                <p style="color: #ccc; margin-bottom: 30px;">
                ...
                <div style="margin-bottom: 30px;">
                    ...
                    <div style="display: flex; gap: 12px; justify-content: center;">
                        <button data-action="amber-login"
                                style="padding: 12px 24px; border: none; border-radius: 8px; cursor: pointer; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); color: #fff; font-weight: bold;">
                            📱 Connect to Amber
                        </button>
                        <button data-action="show-auth"
                                style="padding: 12px 24px; border: none; border-radius: 8px; cursor: pointer; background: var(--bg-hover); color: var(--text-primary);">
                ...
                <div style="font-size: 13px; color: #666; text-align: left; background: color-mix(in srgb, var(--nd-accent) 10%, transparent); padding: 20px; border-radius: 12px; border-left: 3px solid var(--nd-accent);">
                    <p style="color: var(--nd-accent); font-weight: bold; margin-bottom: 12px;">📱 How to get your bunker URI:</p>
                    <ol style="text-align: left; margin: 0; padding-left: 20px; line-height: 1.8; color: #ccc;">
                        <li>Open <strong style="color: var(--nd-accent);">Amber</strong> on your Android device</li>
                        <li>Go to <strong>Settings → Connections</strong></li>
                        <li>Tap "Create Connection" or use existing one</li>
                        <li>Copy the <strong style="color: var(--nd-accent);">bunker://</strong> URI</li>
                        <li>Paste it in the field above</li>
                    </ol>
                </div>

                <div style="font-size: 13px; color: #666; text-align: left; background: color-mix(in srgb, var(--nd-accent) 10%, transparent); padding: 20px; border-radius: 12px; border-left: 3px solid var(--nd-accent); margin-top: 20px;">
                    <p style="color: var(--nd-accent); font-weight: bold; margin-bottom: 12px;">🔐 What is Amber?</p>
                    <p style="color: #ccc; margin-bottom: 12px;">
                        ...
                        Your key never leaves your device - Nosdag requests signatures remotely.
                    </p>
    ...
    if (keyModal) {
        keyModal.innerHTML = `
            <div class="modal-content" style="max-width: 500px;">
                <div class="modal-header" style="color: var(--nd-accent);">🔑 Your New Private Key</div>
                ...
                <div style="margin-bottom: 20px; color: var(--nd-accent); font-weight: bold;">
                ...
                <div style="display: flex; gap: 10px; justify-content: center;">
                    <button id="copyNsecBtn" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); color: #000; border: none; padding: 12px 24px; border-radius: 8px; font-weight: bold; cursor: pointer;">
                        📋 Copy Key
                    </button>
...
export function openZapModal(postId, authorName, moneroAddress, mode = 'choose', customAmount = null, recipientPubkey = null) {
    ...
    // postId '' = a direct user tip (profile Tip button) — same flow, no note reference.
    postId = postId || '';
    ...
    if (mode === 'choose') {
        ...
        details.innerHTML = `
            ...
            <div style="margin-bottom: 16px;">
                <label style="display: block; text-align: center; margin-bottom: 8px; color: var(--nd-accent); font-weight: bold;">
                    Amount (XMR)
                </label>
                <div style="display: flex; gap: 8px; justify-content: center; margin-bottom: 8px;">
                    <button class="preset-amount-btn" data-amount="0.00009" style="background: var(--bg-hover); border: 1px solid var(--nd-accent); color: var(--nd-accent); padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 12px;">0.00009</button>
                    <button class="preset-amount-btn" data-amount="0.00018" style="background: var(--bg-hover); border: 1px solid var(--nd-accent); color: var(--nd-accent); padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 12px;">0.00018</button>
                ...
                <input type="number"
                       ...
                       step="0.00001"
                       min="0.00001"
                       style="width: 100%; padding: 10px; border: 2px solid var(--nd-accent); border-radius: 8px; font-size: 16px; text-align: center; background: var(--bg-tertiary); color: var(--text-primary);">
            ...
            <div id="walletTipSection" style="margin-bottom: 16px; padding: 16px; background: color-mix(in srgb, var(--nd-accent) 10%, transparent); border: 1px solid color-mix(in srgb, var(--nd-accent) 30%, transparent); border-radius: 8px;">
                <div style="text-align: center; margin-bottom: 12px;">
                    <span style="color: var(--nd-accent); font-weight: 600;">💳 Pay with Nosdag Wallet</span>
                </div>
                <button id="walletTipBtn"
                        style="width: 100%; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #fff; padding: 12px 20px; border-radius: 8px; font-weight: bold; cursor: pointer;">
                    Review Tip & Fee
                </button>
            ...
            presetBtns.forEach(btn => {
                btn.onclick = () => {
                    ...
                    presetBtns.forEach(b => {
                        ...
                        b.style.borderColor = b.dataset.amount === 'custom' ? 'var(--border-secondary)' : 'var(--nd-accent)';
                    });
                    btn.style.background = 'var(--nd-accent)';
                    btn.style.borderColor = 'var(--nd-accent)';
                    btn.style.color = '#000';
                };
            });
        ...
        details.innerHTML = `
            <div style="margin-bottom: 16px; text-align: center;">
                ...
                <span style="color: var(--nd-accent);">${escapeHtml(amount)} XMR</span>
            </div>
            <div style="font-size: 12px; color: #666; word-break: break-all; text-align: center; margin-bottom: 16px;">
                ${escapeHtml(moneroAddress)}
            </div>
            ${postId ? `<div style="font-size: 12px; color: #999; text-align: center; margin-bottom: 12px;">
                Note: nosmero.com/n/${escapeHtml(postId)}
            </div>` : ''}
            <div style="text-align: center; margin-top: 16px;">
                <button id="copyPaymentUriBtn"
                        style="background: var(--nd-accent); border: none; color: #fff; padding: 10px 20px; border-radius: 8px; font-weight: bold; cursor: pointer; font-size: 14px; width: 100%; margin-bottom: 12px;">
                    Copy Payment URI
                </button>
                ...
                <div style="display: flex; gap: 8px; justify-content: center;">
                    <button id="copyAddressBtn"
                            style="background: var(--nd-accent); border: none; color: #fff; padding: 8px 12px; border-radius: 6px; font-weight: bold; cursor: pointer; font-size: 12px; flex: 1;">
                        Copy Address
                    </button>
...
async function updateWalletTipButton() {
    ...
    try {
        ...
        if (!hasWallet) {
            ...
            status.textContent = 'Create a tip jar to send tips in-app';
        } else if (!isUnlocked) {
            btn.textContent = 'Unlock & Review';
            status.textContent = 'Enter PIN to unlock Tip Jar';
        } else {
            btn.textContent = 'Review Tip & Fee';
            const balance = await Wallet.getBalance();
            const availableXMR = Wallet.formatXMR(balance.unlockedBalance);
            status.textContent = `Available: ${availableXMR} XMR — nothing is sent until you confirm`;
        }
    } catch (err) {
...
function showWalletPinInput(postId, authorName, moneroAddress, amount, recipientPubkey) {
    ...
    section.innerHTML = `
        <div style="text-align: center; margin-bottom: 12px;">
            <span style="color: var(--nd-accent); font-weight: 600;">🔐 Unlock Tip Jar</span>
        </div>
        <input type="password" id="walletTipPin" placeholder="Enter PIN"
               style="width: 100%; padding: 12px; background: var(--bg-tertiary); border: 2px solid var(--nd-accent); border-radius: 8px; color: var(--text-primary); text-align: center; font-size: 16px; margin-bottom: 12px;">
...
async function sendWalletTip(postId, authorName, moneroAddress, amount, recipientPubkey) {
    ...
    section.innerHTML = `
        <div style="text-align: center; padding: 20px;">
            <div style="color: var(--nd-accent); font-weight: 600; margin-bottom: 12px;">💳 Preparing Tip...</div>
    ...
    try {
        ...
        section.innerHTML = `
            <div style="text-align: center; margin-bottom: 12px;">
                <span style="color: var(--nd-accent); font-weight: 600;">💳 Confirm Tip</span>
            </div>
            <div style="background: var(--bg-tertiary); border-radius: 8px; padding: 12px; margin-bottom: 12px;">
                ...
                <div style="display: flex; justify-content: space-between; border-top: 1px solid var(--border-primary); padding-top: 8px;">
                    ...
                    <span style="color: var(--nd-accent); font-weight: 600;">${totalXMR} XMR${totalUSD}</span>
                </div>
            </div>
            <div style="margin-bottom: 12px;">
                ...
                <div id="walletTipDisclosureWarning" style="display: none; background: rgba(255, 193, 7, 0.1); border: 1px solid rgba(255, 193, 7, 0.3); border-radius: 8px; padding: 12px; margin-top: 8px;">
            ...
            <div style="display: flex; gap: 8px;">
                <button id="confirmTipBtn"
                        style="flex: 1; background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #fff; padding: 12px; border-radius: 8px; font-weight: bold; cursor: pointer;">
                    Confirm & Send
                </button>
        ...
        setTimeout(() => {
            ...
            const disclosureSelect = document.getElementById('walletTipDisclosure');
            disclosureSelect?.addEventListener('change', () => {
                const warn = document.getElementById('walletTipDisclosureWarning');
                if (warn) warn.style.display = disclosureSelect.value === 'verified' ? 'block' : 'none';
            });
            ...
            confirmBtn?.addEventListener('click', async () => {
                ...
                section.innerHTML = `
                    <div style="text-align: center; padding: 20px;">
                        <div style="color: var(--nd-accent); font-weight: 600; margin-bottom: 12px;">💳 Sending...</div>
...
async function publishVerifiedTip(postId, moneroAddress, amount, txHash, txKey, recipientPubkey) {
    try {
        ...
        // Create kind 9736 event for verified tip. A direct user tip (profile Tip button)
        // has no note to reference — never emit an empty ['e',''] tag (relays reject it and
        // it pollutes #e queries); mark the scope instead so #p consumers can tell it apart.
        const tags = [];
        if (postId) tags.push(['e', postId]);           // Referenced note
        else tags.push(['scope', 'profile']);           // Direct user tip, no note
        tags.push(
        ...
        if (signedEvent) {
            ...
            if (pool) {
                // Always include nosmero relay for tip disclosures. Use the shared
                // helper (not an inline copy) so the app:// shell gets wss, not the
                // ws://…:8080 dev port.
                const { getNosmeroRelay } = await import('../relays.js');
                const nosmeroRelay = getNosmeroRelay();
...
async function showQueueBatchConfirm(queue) {
    ...
    section.innerHTML = `
        <div style="text-align: center; padding: 20px;">
            <div style="color: var(--nd-accent); font-weight: 600; margin-bottom: 12px;">💳 Preparing Batch...</div>
    ...
    try {
        ...
        section.innerHTML = `
            <div style="text-align: center; margin-bottom: 12px;">
                <span style="color: var(--nd-accent); font-weight: 600;">💳 Confirm Batch Send</span>
            </div>
            <div style="background: var(--bg-tertiary); border-radius: 8px; padding: 12px; margin-bottom: 12px;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
            ...
            <div style="margin-bottom: 12px;">
                    ...
                    <div id="queuePerTipDisclosure" style="display: none; max-height: 150px; overflow-y: auto; margin-top: 8px;">
                        ${queue.map((item, i) => `
                            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                                <span style="color: var(--nd-accent); font-size: 12px; min-width: 80px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${escapeHtml(item.authorName)}</span>
                ...
                <div id="queueDisclosureWarning" style="display: none; background: rgba(255, 193, 7, 0.1); border: 1px solid rgba(255, 193, 7, 0.3); border-radius: 8px; padding: 12px; margin-top: 8px;">
                    <p style="color: #ffc107; font-size: 12px; margin: 0;">⚠️ Publishes each verified tip permanently — sender, recipient, amount, and the payment proof (tx key) go to public relays and are verifiable on-chain by anyone. Disclosures can't be deleted.</p>
                </div>
            </div>
        ...
        setTimeout(() => {
            ...
            const disclosureWarning = document.getElementById('queueDisclosureWarning');

            function updateQueueDisclosureWarning() {
                if (!disclosureWarning) return;
                const mode = document.querySelector('input[name="queueDisclosureMode"]:checked')?.value || 'all';
                const anyVerified = mode === 'all'
                    ? allSelect?.value === 'verified'
                    : queue.some((item, i) => document.getElementById(`queueDisclosure_${i}`)?.value === 'verified');
                disclosureWarning.style.display = anyVerified ? 'block' : 'none';
            }
            ...
            modeRadios.forEach(radio => {
                radio.addEventListener('change', () => {
                    ...
                    updateQueueDisclosureWarning();
                });
            });

            allSelect?.addEventListener('change', updateQueueDisclosureWarning);
            queue.forEach((item, i) => {
                document.getElementById(`queueDisclosure_${i}`)?.addEventListener('change', updateQueueDisclosureWarning);
            });
...
function generateMoneroQRCode(container, address, amount, postId) {
    const txNote = postId ? `nosmero.com/n/${postId}` : 'tip';
...
function copyMoneroPaymentUri(address, amount, postId, buttonElement) {
    ...
    navigator.clipboard.writeText(moneroUri).then(() => {
        ...
        setTimeout(() => {
            ...
            buttonElement.style.background = 'var(--nd-accent)';
        }, 2000);
    }).catch(err => {
        console.error('Failed to copy:', err);
...
function copyMoneroFieldToClipboard(text, buttonElement, label) {
    navigator.clipboard.writeText(text).then(() => {
        ...
        setTimeout(() => {
            buttonElement.textContent = originalText;
            buttonElement.style.background = 'var(--nd-accent)'; // Back to purple
        }, 2000);
    }).catch(err => {
        console.error('Failed to copy:', err);
...
// Show the tip queue — now a tab inside the Tip Jar modal (wallet-modal.js).
// The legacy #zapQueueModal markup and its wallet-less wizard functions are
// left in place but dormant; nothing routes to them anymore.
export async function showZapQueue() {
    await openWalletModal({ view: 'queue' });
}
...
export function removeFromZapQueue(index) {
    ...
    if (index >= 0 && index < queue.length) {
        ...
        // No refresh here — callers re-render their own surface (the Tip
        // Queue tab in the Tip Jar re-renders via WalletModal.showQueueView)
    }
}
...
function updateZapQueueIndicator() {
    ...
    // Keep the left-rail Tip Queue badge in sync (row may be gone)
    const railBadge = document.getElementById('ndQueueCount');
    if (railBadge) {
        if (queue.length > 0) {
            railBadge.textContent = queue.length;
            railBadge.style.display = 'inline-flex';
        } else {
            railBadge.style.display = 'none';
        }
    }

    // Keep the Tip Queue tab badge in the Tip Jar in sync (the strip only
    // exists once the wallet modal has been opened)
    const tabBadge = document.getElementById('ndQueueTabCount');
    if (tabBadge) {
        if (queue.length > 0) {
            tabBadge.textContent = queue.length;
        ...
        content.innerHTML = `
            <div style="text-align: center; padding: 20px;">
                <div style="margin-bottom: 16px;">
                    ...
                    <p style="color: var(--nd-accent); margin-top: 8px;">Zapping ${escapeHtml(item.authorName)}</p>
                ...
                <div style="display: flex; gap: 12px; justify-content: center;">
                    ...
                    ${currentIndex < queue.length - 1 ? `
                        <button data-action="batch-qr-next" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #fff; padding: 12px 20px; border-radius: 8px; font-weight: bold; cursor: pointer;">
                            Next →
                        </button>
                    ` : `
                        <button data-action="finish-batch-zap" style="background: linear-gradient(135deg, var(--nd-accent), var(--nd-accent)); border: none; color: #fff; padding: 12px 20px; border-radius: 8px; font-weight: bold; cursor: pointer;">
                            Finish & Clear Queue
                        </button>
                    `}
                </div>
