...
import * as CurateUI from '../nosdag/thread-curate-ui.js';

// Phase 5 · Slice 2 — the thread we last rendered, so a curation action can refresh it in place.
let lastThreadEventId = null;
...
export async function openThreadView(eventId, skipHistory = false) {
    try {
        ...
        const [Posts, StateModule, ThreadIndex] = await Promise.all([
            import('../posts.js'),
            import('../state.js'),
        ...
        // Phase 5 · Slice 2 — prefetch each node author's Thread Index so the curation overlay
        // (§6.4) applies synchronously during render. Best-effort; never blocks the view.
        lastThreadEventId = eventId;
        const viewerPubkey = StateModule.publicKey || window.NostrState?.publicKey || null;
        const indexPairs = [];
        (function collectIndexPairs (nodes) {
            for (const n of nodes) {
                if (n.replies && n.replies.length) indexPairs.push({ postId: n.post.id, author: n.post.pubkey });
                if (n.replies) collectIndexPairs(n.replies);
            }
        })(threadTree);
        ...
        async function renderThreadNode(node, depth = 0, parentNode = null) {
            ...
            const childIndent = Math.min((depth + 1) * 20, 100);
            ...
            // Endorsed (or, on an uncurated post, all) replies — in the author's chosen order.
            for (const reply of curated.shown) {
                html += await renderThreadNode(reply, depth + 1, node);
                if (isOwner && curated.hasIndex && ThreadIndex.isApproved(node.post.id, reply.post.id)) {
                    html += CurateUI.ownerControls(node.post.id, reply.post.id, childIndent);
                }
            }
        ...
        // keyed on the conversation root. Author-curated only: following pulls + hosts; the
        // curation overlay above still decides what renders.
        let CT = null;
        let tfRoot = eventId;
        let tfBar = '';
        try {
            CT = await import('../nosdag/consumption-tiers.js');
            tfRoot = CT.rootIdOf(eventId);
            tfBar = CT.followControlHtml(tfRoot);
        } catch (e) { console.warn('[nosdag] thread-follow control unavailable:', e); }

        threadContent.innerHTML = tfBar + (threadHtml || '<div style="text-align: center; padding: 40px; color: #999;">No notes found in thread</div>');
        console.log('[Thread] Set innerHTML, length:', threadContent.innerHTML.length, 'children:', threadContent.children.length);

        // Phase 5 · Slice 2 — wire the curation controls (approve/ignore/block/revoke/reorder).
        CurateUI.wireCurationClicks(threadContent, () => { if (lastThreadEventId) openThreadView(lastThreadEventId, true); });

        // Phase 5 · Slice 4 — wire the follow toggle + register for live repaint while this thread
        // is on screen (a fresh reply under a followed root re-renders it).
        if (CT) {
            CT.wireFollowControl(threadContent, tfRoot);
            CT.markOpenThread(tfRoot,
                () => { if (lastThreadEventId) openThreadView(lastThreadEventId, true); },
                () => StateModule.currentPage === 'thread' && document.getElementById('threadPage')?.style.display !== 'none');
        }
...
export async function openSingleNoteView(eventId) {
    try {
        ...
        threadContent.innerHTML = `
            <div style="margin-bottom: 16px; padding: 12px; background: color-mix(in srgb, var(--nd-accent) 10%, transparent); border-left: 3px solid var(--nd-accent); border-radius: 4px;">
                <div style="color: var(--nd-accent); font-weight: bold;">📍 Direct Note Link</div>
