...
// PIN-600k-derived AES key for the wallet cache. The cache embeds the monero-ts keys file
// (spend-key material), so it sits under the PIN tier — never a view-key-derived key, which
// is a lower-privilege (watch-only grade) secret. The PIN itself is not retained; only this
...
// monero-ts aborts a daemon RPC after this. A full-sync get_blocks.bin batch is multi-MB and over
// slow Tor takes ~20s+; the default is far shorter, so it aborts mid-batch ("Exception in thread
// pool"). A long timeout is harmless on clearnet — fast responses finish well under it and never trip it.
const DAEMON_TIMEOUT_MS = 120000;
...
// Monero block-base58 decode (CryptoNote variant: 11 base58 chars ⇒ 8 bytes, with a fixed
// encoded-length → byte-length map for the trailing partial block). Returns the raw bytes, or null
// if the string isn't a clean base58 decode (bad char / overflowing block / invalid block length).
// Pure + synchronous — just the encoding, no crypto. (L7)
const MONERO_B58_ALPHABET = '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz';
const MONERO_B58_ENC_LEN = [0, 2, 3, 5, 6, 7, 9, 10, 11]; // index = decoded byte count, value = encoded char count
function moneroBase58Decode(str) {
    const full = Math.floor(str.length / 11);
    const rem = str.length % 11;
    const remBytes = rem === 0 ? 0 : MONERO_B58_ENC_LEN.indexOf(rem);
    if (rem !== 0 && remBytes <= 0) return null; // a trailing block of 1 char (or unknown length) is invalid
    const out = new Uint8Array(full * 8 + remBytes);
    const block = (chars, off, n) => {
        let num = 0n;
        for (const ch of chars) {
            const i = MONERO_B58_ALPHABET.indexOf(ch);
            if (i < 0) return false;
            num = num * 58n + BigInt(i);
        }
        if (num >> BigInt(n * 8) !== 0n) return false; // value overflows n bytes
        for (let k = n - 1; k >= 0; k--) { out[off + k] = Number(num & 0xffn); num >>= 8n; }
        return true;
    };
    for (let i = 0; i < full; i++) if (!block(str.slice(i * 11, i * 11 + 11), i * 8, 8)) return null;
    if (remBytes && !block(str.slice(full * 11), full * 8, remBytes)) return null;
    return out;
}
...
function validateAddress(address, context = 'transaction') {
    ...
    // L7: actually decode the base58 and verify the byte structure + mainnet network byte, instead of
    // only checking characters. Standard/subaddress = 69 bytes (1 net + 32 spend + 32 view + 4 csum);
    // integrated = 77 (+8 payment id). Mainnet network bytes: 18 standard, 19 integrated, 42 subaddr.
    // (The cryptographic keccak checksum on the last 4 bytes is enforced by the monero-ts WASM before
    // any tx — this catches malformed/wrong-network addresses earlier, client-side.)
    const decoded = moneroBase58Decode(trimmedAddress);
    if (!decoded || (decoded.length !== 69 && decoded.length !== 77)) {
        throw new Error(`Invalid address for ${context}: not a structurally valid Monero address`);
    }
    if (decoded[0] !== 18 && decoded[0] !== 19 && decoded[0] !== 42) {
        throw new Error(`Invalid address for ${context}: not a mainnet Monero address`);
    }
...
/**
 * True if `address` is a structurally valid mainnet Monero address (format + base58 decode + network
 * byte). The cryptographic checksum is verified by the wallet before sending. (L7)
 * @param {string} address
 * @returns {boolean}
 */
export function isValidMoneroAddress(address) {
    try { validateAddress(address); return true; } catch { return false; }
}
...
// All daemon access goes through main's loopback Monero RPC relay (lib/monero-relay.mjs): it forwards
// to the selected public node (health-checked + fastest-locked, user-overridable), adds CORS, and in
// Tor mode routes through Tor on its own isolated circuit. The renderer points monero-ts at the relay
// URL, so node selection, mixed-content (most public nodes are HTTP-only), CORS and Tor stream isolation
// are all main's concern. No Monero node host is hardcoded here.
let _relayUrl = null;
async function ensureRelayUrl() {
    if (_relayUrl) return _relayUrl;
    try { _relayUrl = await window.nosdag?.monero?.relayUrl?.(); }
    catch (e) { console.warn('[MoneroClient] could not get relay URL:', e?.message); }
    return _relayUrl;
}

function getTorDaemonUri() { return _relayUrl; }
function getClearnetDaemonUri() { return _relayUrl; }
...
/**
 * Get a daemon URI for quick operations (height, etc.) — the loopback relay URL.
 * @returns {string|null}
 */
...
export async function createWallet(pin) {
    ...
    try {
        ...
        isUnlocked = true;
        resetUnlockTimeout();
        await establishCacheKey(pin, ownerPubkey);
...
/**
 * Derive and retain the session cache key while the PIN is in hand.
 * Uses a per-wallet persistent salt (cache_salt on the wallet record, created on first
 * need) so the same key is re-derived across sessions and can decrypt the stored cache.
 * Failure is non-fatal: the wallet still works, it just syncs without a cache.
 * @param {string} pin - User's PIN
 * @param {string} pubkey - Wallet owner's Nostr pubkey
 */
async function establishCacheKey(pin, pubkey) {
    try {
        const walletData = await storage.loadWallet(pubkey);
        let cacheSalt = walletData?.cache_salt;
        if (!cacheSalt) {
            cacheSalt = walletCrypto.generateSalt();
            await storage.updateWalletMeta(pubkey, { cache_salt: cacheSalt });
        }
        cacheKey = await walletCrypto.deriveKey(pin, cacheSalt);
    } catch (e) {
        console.warn('[MoneroClient] Could not establish cache key:', e.message);
        cacheKey = null;
    }
}
...
export async function unlock(pin) {
    ...
    try {
        ...
        await establishCacheKey(pin, currentPubkey);
...
/**
 * Change the wallet PIN: verify the current PIN against the stored keys blob,
 * re-encrypt the keys under the new PIN, and re-key the v2 wallet cache so the
 * next unlock can still delta-sync (legacy view-key caches are PIN-independent).
 * Works locked or unlocked; an unlocked session stays unlocked under the new PIN.
 * @param {string} currentPin - The PIN in use
 * @param {string} newPin - The replacement PIN
 * @returns {Promise<boolean>}
 */
export async function changePin(currentPin, newPin) {
    ...
    const pinCheck = walletCrypto.validatePIN(newPin);
    if (!pinCheck.valid) {
        throw new Error(pinCheck.error);
    }
    ...
    // Verify the current PIN against the stored blob, not the in-memory session
    let keys;
    try {
        keys = await walletCrypto.decryptWalletKeys(
            currentPin,
            walletData.encrypted_keys,
            walletData.iv,
            walletData.salt
        ...
        const remaining = MAX_PIN_ATTEMPTS - result.attempts;
        if (result.lockedOut) {
            throw new Error('Current PIN is incorrect. Tip jar locked for 15 minutes due to too many failed attempts.');
        }
        throw new Error(`Current PIN is incorrect${remaining <= 2 ? ` (${remaining} attempt${remaining !== 1 ? 's' : ''} before lockout)` : ''}`);
    }
    clearPinAttempts();

    // Re-encrypt the keys blob under the new PIN and persist (saveWallet keeps
    // cache_salt, address, restore height and subaddress index from walletData)
    const { encrypted_keys, iv, salt } = await walletCrypto.encryptWalletKeys(newPin, keys);
    await storage.saveWallet({
        ...walletData,
        encrypted_keys,
        iv,
    ...
    // Re-key the v2 cache: decrypt under the old PIN's key, re-encrypt under the new
    // one (same cache_salt). If that fails, drop the cache — the next unlock resyncs.
    try {
        const cacheRec = await storage.loadWalletCache(currentPubkey);
        if (cacheRec && cacheRec.v === 2 && cacheRec.encrypted_data && walletData.cache_salt) {
            const oldKey = await walletCrypto.deriveKey(currentPin, walletData.cache_salt);
            const bytes = await walletCrypto.decryptWalletCacheWithKey(oldKey, cacheRec.encrypted_data, cacheRec.iv);
            if (bytes) {
                const newKey = await walletCrypto.deriveKey(newPin, walletData.cache_salt);
                const enc = await walletCrypto.encryptWalletCacheWithKey(newKey, bytes);
                await storage.saveWalletCache(currentPubkey, {
                    v: 2,
                    encrypted_data: enc.encrypted_data,
                    iv: enc.iv,
                    height: cacheRec.height
                });
            } else {
                await storage.deleteWalletCache(currentPubkey);
            }
        }
    } catch (e) {
        console.warn('[MoneroClient] Cache re-key failed (next unlock will resync):', e.message);
        try { await storage.deleteWalletCache(currentPubkey); } catch { /* best-effort */ }
    }

    // Refresh the in-memory session under the new PIN
    if (isUnlocked) {
        decryptedKeys = keys;
        await establishCacheKey(newPin, currentPubkey);
        resetUnlockTimeout();
    } else {
        walletCrypto.secureWipe(keys);
    }
    ...
    try {
        ...
        try {
            ...
            if (walletCache && walletCache.encrypted_data) {
                ...
                if (walletCache.v === 2) {
                    decryptedBytes = cacheKey
                        ? await walletCrypto.decryptWalletCacheWithKey(cacheKey, walletCache.encrypted_data, walletCache.iv)
                        : null;
                } else {
                    // Legacy pre-v2 cache, encrypted under the view-key-derived key.
                    // Readable once for migration — the next saveWalletCache rewrites it as v2.
                    decryptedBytes = await walletCrypto.decryptWalletCache(
        ...
        if (cachedWalletData && Array.isArray(cachedWalletData) && cachedWalletData.length === 2) {
            ...
            const walletConfig = new MoneroTS.MoneroWalletConfig({
                ...
                keysData: keysData,
                cacheData: cacheData,
                server: { uri: serverUri, timeoutMs: DAEMON_TIMEOUT_MS },
                proxyToWorker: true
            });
...
async function saveWalletCache(wallet, pubkey) {
    ...
    if (!cacheKey) {
        throw new Error('Cache key unavailable (wallet locked?) — skipping cache save');
    }
    const encryptedCache = await walletCrypto.encryptWalletCacheWithKey(
        cacheKey,
        walletDataBytes
    );
