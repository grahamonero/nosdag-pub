...
export async function saveWallet(walletData) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const record = {
            ...
            cache_salt: walletData.cache_salt, // must survive re-saves (PIN change) or the cache key is lost
            ...
            created_at: walletData.created_at || Date.now(),
            updated_at: Date.now()
        };
...
export async function updateWalletMeta(pubkey, updates) {
    ...
    const ALLOWED_META_FIELDS = ['restore_height', 'primary_address', 'nextSubaddressIndex', 'cache_salt'];
    const safeUpdates = {};
