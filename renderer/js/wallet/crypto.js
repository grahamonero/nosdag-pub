...
/**
 * Derive an encryption key from privateViewKey (LEGACY — pre-v2 cache migration reads only;
 * the view key must never protect spend-key material, see encryptWalletCacheWithKey)
...
/**
 * Encrypt wallet cache data under an already-derived session key (PIN-600k tier).
 * The cache embeds the monero-ts keys file — spend-key material — so it must sit under
 * the PIN tier: the view key is a deliberately lower-privilege (watch-only grade)
 * secret and must not be able to reveal the spend key.
 * @param {CryptoKey} key - Non-extractable AES-GCM key derived at unlock (deriveKey)
 * @param {Uint8Array} data - Raw wallet data from getData()
 * @returns {Promise<{encrypted_data: Uint8Array, iv: Uint8Array}>}
 */
export async function encryptWalletCacheWithKey(key, data) {
    const iv = generateIV();
    const ciphertext = await crypto.subtle.encrypt(
        { name: 'AES-GCM', iv: iv },
        key,
...
/**
 * Decrypt wallet cache data with the session cache key (v2 records)
 * @param {CryptoKey} key - Non-extractable AES-GCM key derived at unlock (deriveKey)
 * @param {Uint8Array} encrypted_data - Encrypted cache data
 * @param {Uint8Array} iv - Initialization vector
 * @returns {Promise<Uint8Array|null>} Decrypted wallet data
 */
export async function decryptWalletCacheWithKey(key, encrypted_data, iv) {
    try {
        const plaintext = await crypto.subtle.decrypt(
            { name: 'AES-GCM', iv: iv },
            key,
...
/**
 * Decrypt a LEGACY (pre-v2) wallet cache encrypted under the view-key-derived key.
 * Migration read path only — new caches are written under the PIN-600k session key.
 * @param {string} privateViewKey - Hex string of private view key
 * @param {Uint8Array} encrypted_data - Encrypted cache data
 * @param {Uint8Array} iv - Initialization vector
 * @param {Uint8Array} salt - PBKDF2 salt
