...
// nostr-tools nip44 from the local bundle (window.NostrTools) — no CDN dependency. Kept async so the
// `await getNip44Module()` call sites are unchanged; the bundle's nip44 exposes the same { v2,
// getConversationKey } shape as the CDN module.
async function getNip44Module() {
    const nip44 = window.NostrTools?.nip44;
    if (!nip44) throw new Error('NIP-44 unavailable: NostrTools not loaded');
    return nip44;
}
