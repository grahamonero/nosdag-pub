...
export async function ffmpegStripFile ({ ffmpegBin, inPath, outPath, reencode, format = 'mp4', timeoutMs = 10 * 60_000 }) {
  // Half the cores (floor 1) on both the decode and encode side — at full width a multi-GB
  // H.264 encode starves the UI and everything else on the machine for its whole (legitimately
  // long) runtime. Stream copy is I/O-bound and needs no cap.
  const threads = String(Math.max(1, Math.floor((os.cpus()?.length || 2) / 2)))
  const args = [
    '-y', '-hide_banner', '-loglevel', 'error',
    ...(reencode ? ['-threads', threads] : []),
    '-i', inPath,
    '-map_metadata', '-1', '-map_chapters', '-1', '-fflags', '+bitexact',
    ...(reencode
      ? ['-c:v', 'libx264', '-preset', 'veryfast', '-crf', '23', '-pix_fmt', 'yuv420p', '-threads', threads, '-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart']
      : ['-c', 'copy']),
    '-f', format === 'mov' ? 'mp4' : format, // mov input re-containers to mp4 (same family)
    outPath
  ]
  await new Promise((resolve, reject) => {
    const child = execFile(ffmpegBin, args, { timeout: timeoutMs }, (err, _stdout, stderr) => {
      if (err) reject(new Error(`ffmpeg failed: ${String(stderr || err.message).trim().slice(0, 300)}`))
      else resolve()
    })
    // Below-normal priority (os.setPriority maps it per-platform); a courtesy only — never
    // fail the strip over it.
    try { os.setPriority(child.pid, 10) } catch { /* unsupported platform or already exited */ }
  })
}
