...
export function connectKubo (apiPort) {
  ...
  return {
     ...
     *  files.stat on an /ipfs/<cid> path returns CumulativeSize without touching MFS. The timeout
     *  is load-bearing: stat on a partially-local DAG Bitswap-searches for the missing blocks
     *  and would otherwise hang the IPC forever (e.g. a hosted peer that went offline mid-walk). */
    dagSize: async (cidStr, timeoutMs = 30000) => {
      const st = await client.files.stat(`/ipfs/${cidStr}`, { timeout: timeoutMs })
