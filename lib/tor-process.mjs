...
// GUI-launched mac apps inherit launchd's minimal PATH (no /opt/homebrew/bin), so a bare
// spawn('tor') misses a Homebrew/MacPorts install that works fine from a terminal. Probe
// the PATH ourselves, then the mac package-manager dirs; fall back to bare 'tor' so a
// genuinely missing binary still surfaces spawn's classified ENOENT error.
function findTorBin () {
  ...
  const dirs = (process.env.PATH || '').split(path.delimiter).filter(Boolean).concat(extra)
  for (const d of dirs) {
    const p = path.join(d, 'tor')
